...
import { ExpectedMessages, MenuOptions } from '@/e2e/src/testData';
...
test(
  ...
  async ({
    ...
    let prompt: Prompt;
    await test.step('Prepare a new prompt', async () => {
      prompt = promptData.prepareDefaultPrompt();
      await localStorageManager.setPrompts(prompt);
    });

    await test.step('Open prompt dropdown menu and choose "Share" option', async () => {
    ...
    await test.step('Click on "Cancel" button in modal window and verify no shared icon appears on prompt icon', async () => {
    ...
    await test.step('Open Share modal again, click outside modal window area and verify no shared icon appears on prompt icon', async () => {
      await prompts.openPromptDropdownMenu(prompt.name);
      await promptDropdownMenu.selectMenuOption(MenuOptions.share);
    ...
    await test.step('Open Share modal again, click on "Copy" button in modal window, close it and purple shared icon appears on prompt icon', async () => {
      await prompts.openPromptDropdownMenu(prompt.name);
      await promptDropdownMenu.selectMenuOption(MenuOptions.share);
      await shareModal.copyLinkButton.click();
      await shareModal.closeButton.click();
      await prompts.getPromptArrowIcon(prompt.name).waitFor();
      const arrowIconColor = await prompts.getPromptArrowIconColor(prompt.name);
      expect
        .soft(arrowIconColor[0], ExpectedMessages.sharedIconColorIsValid)
        .toBe(Colors.textSecondary);
    });

    await test.step('Open Share modal again, click on "Copy" button in modal window and verify only one purple shared icon is shown on prompt icon', async () => {
      await prompts.openPromptDropdownMenu(prompt.name);
      await promptDropdownMenu.selectMenuOption(MenuOptions.share);
      await shareModal.copyLinkButton.click();
      await shareModal.closeButton.click();
...
test(
  ...
  async ({
    ...
    await test.step('Close modal window and verify purple shared icon appears on prompt icon', async () => {
      await shareModal.closeButton.click();
      await prompts.getPromptArrowIcon(prompt.name).waitFor();
      const arrowIconColor = await prompts.getPromptArrowIconColor(prompt.name);
      expect
    ...
    await test.step('Update prompt details and verify shared icon stays on prompt icon', async () => {
