...
export default function ShareIcon({ isShared, isPublished, isHighlited, highlightColor, size = 10, children }: ShareIsonProps) {
  ...
  return (
    <div className="relative">
        ...
        <AdditionalIcon size={size} width={size} height={size} className={classNames(
            getByHighlightColor(
                highlightColor,
            ...
            'p-[1px]',
            isHighlited && getByHighlightColor(highlightColor, 'bg-green/15', 'bg-violet/15', 'bg-blue-500/20')
            )}/>
        </div>
    </div>
  );
