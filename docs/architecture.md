...
**Generation lifecycle and admission (`generation-registry`).** Every admitted entry carries an explicit `GenerationLifecycleState` (`active` → `cancel_requested` → `finalizing` → `settling` → `released`) and, when cancellation is requested, one `GenerationCancelReason` (`user_stop` | `stale_expiry` | `max_duration` | `shutdown`). `register()` mints a process-local operation identity — a lease, distinct from the client-supplied `generationId` — and admits a key only when no entry (in any non-`released` state) already occupies it: **a present entry is always a conflict**, rejected with `409` before any `await`, so no replacement can ever overlap an older worker's ability to persist. Stale handling (a sweep run on `register()` only, with the threshold derived as `max(30 min, MAX_GENERATION_DURATION_MS) + grace` so it can never pre-empt the max-duration timer) _requests cancellation and aborts the upstream_ rather than removing the entry — the owning worker still performs its own single terminal save and releases the entry itself. Only a `user_stop` reason is ever persisted or reported as a user Stop; `stale_expiry` and `max_duration` are always a non-user abort. If a dispatched terminal write never settles within `GENERATION_FINALIZE_TIMEOUT_MS`, the entry releases its attach subscribers, timer and runtime tracking but **retains its registry key** (`state="settling"`) — deliberately, because releasing the key would readmit the overwrite this design prevents; recovery is the write eventually settling, or a process restart. This is a stated limitation, not an oversight — see `openspec/specs/generation-registry/spec.md` for the full state machine and `docs/observability.md` for how it is surfaced on the generations gauge.
