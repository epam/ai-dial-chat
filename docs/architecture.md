...
| `HalloweenContext`            | The Halloween easter egg, gated on `halloweenEnabled` and the exact start-page route `/`: pumpkin clicks randomly choose ghosts, one connected web over almost the whole start screen and history, bats, a cat with wisps, or flying witches, without consecutive repeats. Every celebration notification reveals the secret phrase, which only intercepts the start-page composer. Navigation clears effects; existing conversations and editor previews remain ordinary chats. In-memory only; nothing is persisted or sent anywhere. Unlike the other contexts, its consumer hook resolves to a permanently disabled easter egg outside the provider instead of throwing |
