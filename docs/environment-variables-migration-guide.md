...
The single `AUTH_PROVIDERS` JSON-array environment variable has been **removed** (breaking change, no dual-path support). For each object that used to be an entry in that array, map its fields to the new provider-specific variables:

| Old `AUTH_PROVIDERS[i]` field | New variable(s)                                                                 | Notes                                                                                          |
| ------------------------------ | -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `id`                            | *(none — fixed in code)*                                                          | The id is now one of the 9 hardcoded provider ids (`auth0`, `azure-ad`, `azure-b2c`, `gitlab`, `google`, `keycloak`, `ping-id`, `cognito`, `okta`); pick the matching provider's variable set. |
