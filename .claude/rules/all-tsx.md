...
**Always import the 2.0 component.** Reach for a `Dial*` component only when the MCP lookup shows it has no 2.0 replacement (e.g. `DialPagination`, `DialSlider`, `DialGrid`, `DialFormPopup`, `DialNoDataContent`, `DialFileManager` currently have none). `DialTooltip`, `DialEllipsisTooltip`, `DialCheckbox`, `DialRadioButton`, `DialRadioGroup` and `DialSegmentedControl` gained 2.0 counterparts in ui-kit 0.14 — use `Tooltip`, `EllipsisTooltip`, `Checkbox`, `Radio`, `RadioGroup`, `SegmentedControl`.
