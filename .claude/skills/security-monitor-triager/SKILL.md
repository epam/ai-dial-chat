...
- `payload.findings[]` — one per reviewed advisory: `{severity (=adjusted), file?, line?, message, suggested_fix?, osv_id, verdict, original_severity, adjusted_severity}`. `message` is one line: `<VERDICT> — <osv_id> <package>@<version>: <evidence: source→sink, mitigation>`.
- `payload.news_review[]` — news verdicts `{title, source, producer_tag, verdict, note}`.
