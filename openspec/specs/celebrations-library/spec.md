# celebrations-library Specification

## Purpose

The publishable `@epam/ai-dial-celebrations` package: its entry points, the provider contract through which hosts supply every external input, per-event scene and decoration selection, labels, DOM anchors, host isolation, Storybook coverage, and how DIAL Chat integrates it through an app-level adapter.
