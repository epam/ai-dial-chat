# chat-hooks-favorites-state Specification

## Purpose

A reusable `useFavoriteEntitiesState` hook exported by `@epam/ai-dial-chat-hooks` that encapsulates the aggregate-load-then-optimistic-toggle behavior of `FavoriteApplicationsContext`, with all server-api and context knowledge kept at the app edge.
