# prompt-favorites Specification

## Purpose

Prompt favourites: the `prompts.installed` user-config section and its migration to config version 4, the `PATCH /api/v1/user-config/prompts` toggle, and how the frontend folds prompt favourites into the single favourites set.
