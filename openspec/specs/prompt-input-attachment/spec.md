...
`CatalogView`'s existing "Use in chat" primary action (`handleUseInChat`, `CatalogEntityType.Prompt`) SHALL apply the same `{{param}}` token check used everywhere else in this capability, rather than always navigating with the raw, unresolved `content`. Model/Agent/Toolset items are unaffected — this requirement only changes prompt handling.
...
- **Catalog "Use in chat" entry point**: `CatalogView.handleUseInChat`'s existing prompt branch already resolves `content` (fetching it if the list item didn't carry it) and hands off to the chat route via router state, exactly the mechanism the parameter-less path reuses. The token check and popup hand-off slot into that same branch — no second prompt-fetch path is introduced, and non-prompt items (`Model`/`Agent`/`Toolset`) are untouched.
