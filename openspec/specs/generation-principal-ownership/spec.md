...
How the backend derives a stable, server-verified owner for an in-flight generation under each authentication mode, how owner keys are composed so distinct principals can never collide, and the isolation guarantees that follow for start, stop, and attach.
