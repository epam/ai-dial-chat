...
`ListScheduledTasksResponseDto` SHALL include optional `count`, `limit`, `offset`, `next`, and `previous` fields mirroring the upstream paginated envelope (`{ count, limit, offset, results, next, previous }`), so a caller can detect that more schedules exist beyond the current page. These fields are informational only in this change — the endpoint still returns a single upstream page and does not yet accept client-supplied `limit`/`offset` query parameters; a caller with more than the upstream page size worth of schedules will not see the remainder until a follow-up change adds paged fetching.
