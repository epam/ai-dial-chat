# catalog-prompt-entity-type Specification

## Purpose

The `CatalogEntityType.Prompt` member and everything `libs/catalog` renders generically for it: the Prompts tab label and position, the read-only Content details tab with markdown and highlighted `{{placeholders}}`, the two-tab details panel, primary-action and publish defaults, the host predicate that suppresses unshare/revoke per item, and search/sort/filter/column behaviour.
