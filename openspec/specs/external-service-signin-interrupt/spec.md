...
Core's `params.url` is shaped `applications/{bucket}/{app}/external_services/{name}`, where `{name}` is a real, distinct identifier — it keys the application's own `external_services` map (`external-service-authentication`) and DIAL Core requires the *full* url as the sign-in/sign-out scope id (confirmed by Core rejecting a bare application id with `400 Invalid external service scope id`). `parseExternalServiceUrl` (`apps/chat/src/utils/external-services.ts`) SHALL split `params.url` into `appId` (everything before `/external_services/`, used to fetch the application's details) and `serviceName` (everything after, keys `applicationDetails.externalServices`). If `params.url` contains no `/external_services/` segment, the event SHALL be ignored (dropped) rather than guessing a service name — this shape is required, not optional.
...
- **THEN** the row shows a fallback label derived from the event's `serviceName` instead of blocking rendering
