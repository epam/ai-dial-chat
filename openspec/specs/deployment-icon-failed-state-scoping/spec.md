# Spec: deployment-icon-failed-state-scoping

## Purpose

Ensures `DeploymentIcon`'s failed-image fallback state is scoped to the specific `src` that failed, so a previous source's failure never leaks onto a newly displayed, successfully-loading source.
