# conversation-custom-view-state Specification

## Purpose

TBD - created by archiving change persist-conversation-level-annotations. Update Purpose to describe the actual capability.
...
resolution described in the `quotations-citation-markdown` capability. It
SHALL NOT be passed as the hook's primary `groups` argument.
