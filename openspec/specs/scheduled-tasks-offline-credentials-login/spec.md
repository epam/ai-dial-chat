# Spec: scheduled-tasks-offline-credentials-login

## Purpose

Defines the Scheduled Tasks list page's proactive offline-credentials login flow: the route-scoped status check against DIAL Core's offline-credentials endpoint, the non-blocking inline login-required banner (and its retry/accessibility behavior), and the OAuth login flow that reuses the shared toolset OAuth popup/callback/`BroadcastChannel` infrastructure.
