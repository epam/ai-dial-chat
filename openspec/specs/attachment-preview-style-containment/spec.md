...
## Purpose
Specifies the application-level guarantee that vendor CSS pulled in by a lazily loaded attachment-preview engine cannot alter layout or typography outside the preview surface — in any route, at any viewport width, for the rest of the session. A preview engine's stylesheet is injected when its chunk loads, after the application's own, and is never unloaded; when that stylesheet carries a CSS reset or duplicates the application's utility class names, it wins on document order at equal specificity and the damage is permanent for the session. This capability constrains how such a stylesheet is loaded, and how the guarantee is verified.
