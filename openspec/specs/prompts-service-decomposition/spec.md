# Spec: prompts-service-decomposition

## Purpose

Backend decomposition of the monolithic `PromptService` (`apps/chat-api/src/prompts/prompt.service.ts`) into focused injectable services plus a thin facade, mirroring the earlier `ConversationService` and `DeploymentsService`/`ToolsetsService` splits, so shared resource I/O, personal prompt CRUD, public/organisation reads, and folder lifecycle each live in their own testable service while the observable REST contract stays unchanged.
