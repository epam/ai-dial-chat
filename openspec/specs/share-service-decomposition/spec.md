# Spec: share-service-decomposition

## Purpose

Backend decomposition of the monolithic `ShareService` (`apps/chat-api/src/share/share.service.ts`) into focused injectable services plus a thin facade, mirroring the earlier `ConversationService`, `FilesService`, and `DeploymentsService`/`ToolsetsService` splits, so invitation issuance/acceptance and post-grant share management each live in their own testable service while the observable REST contract and cache-invalidation behavior stay unchanged.
