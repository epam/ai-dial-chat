# chat-hooks-toolset-login Specification

## Purpose

Host-agnostic toolset login hooks published from `@epam/ai-dial-chat-hooks`:
`useToolsetLogin`, which drives the API-key and OAuth login paths and resolves a
discriminated success / failure / popup-blocked / cancelled outcome, and the OAuth
callback-completion hook that runs inside the popup to validate state, exchange the
authorization code, and report the result back to the opener. Both take their API access
as injected callbacks and return state rather than rendering, translating, or raising
notifications; routing, i18n, and API dispatch stay with the host.
