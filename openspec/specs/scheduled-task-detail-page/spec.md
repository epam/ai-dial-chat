...
### Requirement: Detail page header shows back navigation and title only

The detail page header SHALL render a back-navigation control and the task's `displayName` as its title. Activating the back control SHALL navigate to `ROUTES.ScheduledTasks`. The header SHALL NOT render Edit, Delete, Active-toggle, or Run-now controls in this iteration.
...
#### Scenario: Header shows only back and title

- **WHEN** the detail page renders with a successfully loaded task
- **THEN** the header contains a back control and the task's `displayName`, and no Edit/Delete/Active-toggle/Run-now control is present
...
`libs/scheduled-tasks` SHALL export a presentational `ScheduledTaskDetailView` component accepting only props: localized label strings, detail field values (`description`, model display value, schedule label), either `instructionsMarkdown: string` or a `renderInstructions: (markdown: string) => ReactNode` callback, a runs list plus `{ runsHasMore, runsIsLoadingMore, runsSkeletonCount, onRunsLoadMore }`, top-level `isLoading`/`error` flags and their History-scoped counterparts, and an `onBack` callback. The component SHALL NOT import `@epam/chat-api-client`, any routing module, i18n, or auth/env/analytics modules — all host/external knowledge is passed in via props per the repo's library-isolation rule.
