# Spec: bff-header-token-auth

## Purpose

Lets non-browser callers (service integrations, scripts, tools holding a valid OIDC
access token) authenticate to `apps/chat-api` with a bearer token in the `Authorization`
header, as a second credential source alongside the existing encrypted-cookie session
flow. Defines the pluggable authentication-strategy chain that both `SessionGuard` and
`OptionalSessionGuard` use, the precedence and no-fallback rules between header and
cookie auth, local JWKS-backed token verification, the CSRF exemption for
header-authenticated requests, and per-request bucket resolution/caching for
header-authenticated callers — all without changing any observable behavior of the
existing cookie-session flow.
