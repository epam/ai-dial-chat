# catalog-custom-empty-state Specification

## Purpose

Defines `@epam/ai-dial-catalog`'s `Catalog.renderEmptyState` render prop and the exported `CatalogEmptyStateContext` it receives, letting a host replace the Browse section's default empty state (`PanelEmptyState`) with a custom node driven by live search/filter/tab state, while preserving the default empty state — and `CardGrid`'s/`ListView`'s own standalone behavior — for hosts that don't opt in.
