...
#### Scenario: OverlayFeature has exactly 40 members

- **WHEN** `Object.values(OverlayFeature)` is inspected
- **THEN** it has exactly 40 unique string values, including `'chat-settings'`, `'voice-input'`, `'header'`, `'likes'`, `'hide-new-conversation'`, `'live-chat-interaction'`, `'prompts'`, `'skills'`, `'file-manager'`, `'hide-change-agent'`, `'hide-conversations-filter'`, and `'hide-keyboard-shortcuts'`
