...
aggregate cost-limit cards, their public prop/label/color contract, status-driven visual
treatment, accessibility, and responsive/RTL behavior. The library never interprets raw DTOs,
formats currency, or computes percentages — that stays in `apps/chat` (see the `usage-data-hook`
capability).
