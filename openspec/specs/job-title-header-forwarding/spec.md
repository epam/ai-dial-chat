# job-title-header-forwarding Specification

## Purpose

Preserve the caller's OIDC job title in authenticated session state and forward
it to DIAL Core on requests whose behavior, limits, analytics, or audit context
can depend on that title. Keycloak may supply the value through UserInfo when
it is absent from the ID token.
