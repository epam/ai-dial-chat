# skills-catalog-context Specification

## Purpose

Defines `SkillsContext` as the app's single owner of skill listing state: its exposed value, how the personal and organisation listings load independently, recursive pagination under a bounded page cap, exclusion of grouping folders, feature-flag short-circuiting, and where the provider mounts.
...
The context SHALL expose no refetch callback. Skill listing performs no mutation, so nothing would call one; an exported function no caller invokes is dead API. A later change that adds a mutation adds the refetch it needs alongside it.
