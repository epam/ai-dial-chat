# prompts-context Specification

## Purpose

`PromptsContext` as the single owner of prompt and prompt-folder state: the provider/hook shape, namespace-level degradation of the aggregate response at the BFF boundary, and refetch-rather-than-patch on every mutation.
