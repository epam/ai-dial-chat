# catalog-content-file-preview Specification

## Purpose

Defines the generic, entity-agnostic typed file preview rendered by `libs/catalog`'s Content tab: the `CatalogContentFilePreview` model and its four variants, the `onLoadContentFilePreview`/`renderContentFilePreview` host contract and its precedence over the legacy `onLoadContentFile`, the stale-request guard, and object-URL ownership and cleanup.
