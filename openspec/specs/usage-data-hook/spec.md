...
Defines the empty `Usage` tab container and the `useUsageData` hook: fetching the current user's
limits and usage from the existing BFF endpoints via the existing `server-api` wrappers, exposing
loading/error state, and the library-isolation contract that keeps this data access out of `libs/*`.
