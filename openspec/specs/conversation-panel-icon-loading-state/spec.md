...
This ensures the `iconBefore` slot retains its 24 × 24 px footprint with no layout shift when the real icon arrives, and delegates animation and theming to the design system. `color="var(--bg-layer-4)"` is consistent with the token used by skeleton rows in `ConversationPanel` when `isLoading` is true.
