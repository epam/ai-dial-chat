# Spec: share-popover-qr-actions

## Purpose

Lets a user take the share link's QR code out of `SharePopover`'s QR view: copy it to the clipboard as a PNG image (falling back to the share URL as text) or download it as a PNG file, with accessible feedback and host-overridable labels, while `libs/share` stays free of host integration details.
