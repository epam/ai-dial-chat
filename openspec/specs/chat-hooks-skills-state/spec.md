# chat-hooks-skills-state Specification

## Purpose

A reusable `useSkillsState` hook exported by `@epam/ai-dial-chat-hooks` that encapsulates the gated fetch behavior of `SkillsContext`, with all feature-flag and auth gating kept at the app edge via injected `enabled`/`ready` booleans.
