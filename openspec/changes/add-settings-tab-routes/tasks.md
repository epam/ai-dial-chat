...
- [ ] 1.1 Add `SettingsTab = '/settings/:tab'` to the `ROUTES` enum in
...
- [ ] 1.2 Add a `buildSettingsTabPath(tab: SettingsTabs): string` helper beside the enum, returning
      the concrete path for a tab. Give it JSDoc per the app conventions.
- [ ] 1.3 Add a spec at `apps/chat/src/types/tests/routes.spec.ts` (or the app's existing utils
...
- [ ] 2.1 Register `ROUTES.SettingsTab` in `apps/chat/src/app/app.tsx` alongside the existing
      `ROUTES.Settings` entry, reusing the same lazy `SettingsPage`, `RouteErrorBoundary`,
...
- [ ] 2.2 Change the `ROUTES.Settings` element to `<Navigate replace />` to the default tab's path,
      keeping it behind the same flag gate.
- [ ] 2.3 In `apps/chat/src/pages/SettingsPage/SettingsPage.tsx`, delete the `useState` selection
      ...
      URL into state.
- [ ] 2.4 Make `onSelect` navigate to `buildSettingsTabPath(id)` with a normal push, so Back steps
      between tabs.
- [ ] 2.5 Redirect an unresolvable segment with `<Navigate replace />` to the default tab's path,
...
- [ ] 2.6 Confirm no change was needed in `apps/chat/src/hooks/useSettingsTabConfig.tsx` or
...
- [ ] 2.7 Add or extend `apps/chat/src/pages/SettingsPage/tests/SettingsPage.spec.tsx`: the route
...
- [ ] 2.8 Cover the route gating where the app's existing route tests live: `/settings` redirects to
      the default tab with the flag on, and both `/settings` and `/settings/usage` redirect to
      `ROUTES.Root` with the flag off.
...
- [ ] 3.3 Architecture guard: confirm `libs/catalog` gained nothing — no router import, no settings
...
- [ ] 4.1 Check the footer link for logical properties only, and confirm it is a real `<a>` through
      `<Link>` rather than a click-handling `<div>`, so it is keyboard-reachable and announces as a
      link.
- [ ] 4.2 Confirm focus behaviour on navigation: the popover closing must not strand focus on a
      removed node.
...
- [ ] 5.1 Update `docs/architecture.md` for the new `ROUTES` member and the settings route shape —
      `AGENTS.md` requires it in the same change for any `ROUTES` enum or route-folder change.
- [ ] 5.2 Check whether `docs/technical-requirements.md` describes the Settings page or the usage
...
- [ ] 6.1 Run `npm run verify:full` once and fix anything it surfaces that belongs to this change.
- [ ] 6.2 Confirm the merge boundary held: `git diff` shows no change to
