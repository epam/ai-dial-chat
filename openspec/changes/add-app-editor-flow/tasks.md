...
- [x] 8.2 Create `apps/chat/src/pages/AppsEditor/GeneralForm.tsx` — two-column layout: left column has `DialInput` (name, icon URL, version), `DialTextarea` (description), `DialTagInput` (topics), footer with `DialNeutralButton` (Cancel) + `PrimaryButton` (Next); right column (`bg-layer-1`) shows a live `<Card>` preview centered in the area, driven by `useMemo`-derived `CatalogItem` from form state; call `createApplication` on submit, invoke `onCreated(appId)` on success, show inline error on failure
