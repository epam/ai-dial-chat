...
```ts
/** When provided, adds a "Prompts" item (IconPrompt) above "Chat settings"; renders the app-owned overlay content on desktop (dropdown-style) and mobile (bottom sheet). */
promptsMenuOverlay?: (onClose: () => void, onBack?: () => void) => ReactNode;
promptsMenuTitle?: string; // "Prompts", i18n default
```

**Revised — desktop replaces the main menu rather than nesting a popover inside it, and gets its own `onBack`:** the original plan was a nested popover anchored to the "Prompts" row (the same "second level opens beside/inside the first, both stay open" shape `AddAttachmentButton`'s existing "Tools" item uses via `DropdownItem.children`). Building it revealed a hard ui-kit limitation: every nested `children` item is unconditionally rendered inside a fixed `h-[40px] truncate` `<button>` (confirmed by reading the ui-kit's `Dropdown` submenu renderer — there is no `PlainText`/rich-content branch at the nested level, unlike the top-level items list), so the Favorites panel's header/list/star buttons/Browse button got clipped and, worse, nested `<button>`-inside-`<button>` HTML. `apps/chat/src/components/Navigation/UserMenu.tsx` — pointed to as a possible reference — doesn't demonstrate a working alternative either: its nested `children` submenus ("language", "keyboard shortcuts") are flat single-line rows, and its one rich-content row (`PlainText` identity block) is a top-level item, not nested. Fixing this properly (both the main menu and the Prompts panel open and visible side by side) needs a newer ui-kit release and is deferred.

Until then, desktop uses the same `Dropdown`/`renderOverlay` mechanism `ModelSelectorControl` already uses for `modelPickerOverlay`, anchored to the same trigger as the main menu: clicking "Prompts" closes the main menu (`isDesktopMenuOpen=false`) and opens this overlay (`isPromptsOverlayOpen=true`) in its place, which gets the standard top-level Dropdown chrome (rounded/background/shadow) that a nested flyout never gets. Because this replaces the main menu instead of coexisting with it, `promptsMenuOverlay` is called with a second callback, `onBack`, that closes the overlay and reopens the main menu; `FavoritePromptsPanel` renders it as an optional back chevron in its own header (same `onBack?` pattern already used by `PromptParametersPopup`).

Mobile does **not** get a new `PromptsBottomSheet`. It reuses the existing generic `BottomSheetShell` (already used by `ModelSelectorControl` for `modelPickerOverlay`), which already provides the exact back-navigation shell needed (`onBack` returns to the main Add sheet) — avoiding a second bespoke shell alongside `ToolsBottomSheet`'s. Mobile calls `promptsMenuOverlay` with only `onClose` (no `onBack`), since `BottomSheetShell`'s own back arrow already covers it and `FavoritePromptsPanel` would otherwise render two back buttons.
...
- None outstanding. (Resolved: mobile bottom-sheet stacking reuses `BottomSheetShell` directly rather than a new `PromptsBottomSheet` — see Decision 2 — and the desktop nested-popover approach was replaced with a menu-replacing overlay plus its own `onBack`, pending a ui-kit update that would allow both levels to stay open simultaneously.)
- Still deferred: implement CatalogView's `visibleTypes` selector-mode filter (Decision 3 / task group 4) — the Prompts-only "Use prompt" browse modal is not yet wired up.
