...
`CatalogView`'s existing "Use in chat" primary action (`handleUseInChat`, `CatalogEntityType.Prompt`) SHALL apply the same `{{param}}` token check used everywhere else in this change, rather than always navigating with the raw, unresolved `content`. Model/Agent/Toolset items are unaffected — this requirement only changes prompt handling.

When the selected prompt's content contains one or more `{{param}}` tokens, clicking "Use in chat" SHALL close the Catalog page (navigate away the same way the parameter-less path already does) and open the same "Prompt parameters" popup defined below, pre-loaded with that prompt, instead of inserting the unresolved content. The prompt travels to the chat route the same way parameter-less content already does today (router state, not a query param — see `handleUseInChat`'s existing rationale), so no new persistence or backend call is introduced.
...
- **RTL/direction**: the back chevron (`IconChevronLeft`), used in the Prompt parameters popup header (the Favorites panel no longer renders one — see the desktop nested-submenu note above), requires `rtl:scale-x-[-1]`; the star icon, close icon, and prompt icon are symmetric/conceptual and are not mirrored. All layout in the new lib components uses logical Tailwind classes (`ms-*`/`me-*`/`ps-*`/`pe-*`/`text-start`/`text-end`) per `.claude/rules/rtl.md`.
...
- **Catalog "Use in chat" entry point**: `CatalogView.handleUseInChat`'s existing prompt branch already resolves `content` (fetching it if the list item didn't carry it) and hands off to the chat route via router state, exactly the mechanism the parameter-less path in this change reuses. The token check and popup hand-off added by this change slot into that same branch — no second prompt-fetch path is introduced, and non-prompt items (`Model`/`Agent`/`Toolset`) are untouched.
