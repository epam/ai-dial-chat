...
- [ ] 1.1 In `apps/chat/src/server-api/base.ts`, add `credentials: 'include'` to the `fetch` options inside the shared `request()` helper. Keep the change additive (do not touch existing signatures or call sites).
- [ ] 1.2 In `apps/chat/src/server-api/base.ts`, export an `UnauthorizedError` class extending `Error` with `readonly status: 401` and a `readonly url: string` field; set `name = 'UnauthorizedError'`.
- [ ] 1.3 In `apps/chat/src/server-api/base.ts`, replace the generic 401 path inside `request()` so that on `response.status === 401` it throws `new UnauthorizedError(url)` instead of a generic `Error`. Non-401 non-OK statuses keep the existing `Error('Request failed with status …')` message.
- [ ] 1.4 In `apps/chat/src/server-api/base.ts`, add a module-scoped listener registry: `const listeners = new Set<(url: string) => void>()` plus an exported `onUnauthorized(listener)` function that returns a cleanup callback (`() => listeners.delete(listener)`). Inside `request()`, invoke each listener synchronously **before** throwing the `UnauthorizedError`.
- [ ] 1.5 Extend the `ApiEndpoints` enum in `apps/chat/src/server-api/base.ts` with `AUTH_ME = '/api/v1/auth/me'`, `AUTH_PROVIDERS = '/api/v1/auth/providers'`, and `AUTH_LOGOUT = '/api/v1/auth/logout'`. Do **not** add a `LOGIN` constant (the URL is built per-provider at the call site).
- [ ] 1.6 Update `apps/chat/src/server-api/base.spec.ts` with co-located Vitest cases (use `vi.spyOn(globalThis, 'fetch')`, no `data-testid`):
...
- [ ] 1.7 Verify the slice in isolation: `pnpm nx test chat` and `pnpm nx lint chat`.

## 2. Slice 2 — UserContext provider and useUser hook

- [ ] 2.1 Create `apps/chat/src/context/UserContext.tsx`. Mirror the structure of `apps/chat/src/context/ThemeContext.tsx`:
...
- [ ] 2.2 Export `useUser(): UserContextType` from the same file; throw `Error('useUser must be used within a UserProvider')` when the context is `undefined`.
- [ ] 2.3 Import `UserProfile` from `@epam/chat-shared` — do not redefine it locally.
- [ ] 2.4 In `apps/chat/src/main.tsx`, wrap the existing `<ThemeProvider>` with `<UserProvider>` so the tree becomes `<BrowserRouter><UserProvider><ThemeProvider>…</ThemeProvider></UserProvider></BrowserRouter>`. Do **not** add `<Routes>` yet (that lands in slice 3).
- [ ] 2.5 Add `apps/chat/src/context/UserContext.spec.tsx` with Vitest + `@testing-library/react`. Use role/label/text queries only. Cover:
...
- [ ] 2.6 Verify the slice: `pnpm nx test chat` and `pnpm nx lint chat`.

## 3. Slice 3 — i18n keys batch

- [ ] 3.1 Add the `auth` namespace to `apps/chat/src/i18n/locales/en.json` (and any other locale files present in the repo at the time of implementation) with the following keys and English copy:
...
- [ ] 4.1 Create `apps/chat/src/hooks/useAuthRedirect.ts`. JSDoc at the top explains WHY the policy is centralised here (per the design doc D3). The hook:
  ...
  - If exactly one provider, status is `'unauthenticated'`, and the current pathname is not `/login`: `window.location.assign('/api/v1/auth/login/' + encodeURIComponent(id))`.
  - If two or more providers and status is `'unauthenticated'` and current pathname is **not** `/login`: `navigate('/login', { replace: true })`.
  - If `status === 'authenticated'` and current pathname is `/login`: `navigate('/', { replace: true })`.
  - Returns `void`.
...
- [ ] 4.5 Add `apps/chat/src/hooks/useAuthRedirect.spec.ts`. Cover:
  - 4.5.a Mocked `window.location.assign` is called once with the correct URL when one provider, `status='unauthenticated'`, and pathname is not `/login`.
  - 4.5.b `navigate('/login', { replace: true })` is called when ≥ 2 providers and `status='unauthenticated'` and the pathname is `/conversation`.
  - 4.5.c No navigation happens when `status='loading'`.
  - 4.5.d `navigate('/', { replace: true })` is called when `status='authenticated'` and pathname is `/login`.
...
- [ ] 5.1 Create `apps/chat/src/pages/Login.tsx` as a functional component named `LoginPage`. It:
  ...
  - On success: renders an `<h1>` with `t('auth.loginTitle')`, a paragraph with `t('auth.loginDescription')`, and one `<a>` per provider whose `href` is `'/api/v1/auth/login/' + encodeURIComponent(id)` and whose accessible name is `t('auth.providerButtonLabel', { provider: label })`. Anchor tags MUST NOT be React Router `<Link>`.
...
- [ ] 5.3 Add `apps/chat/src/pages/Login.spec.tsx`. Cover:
  ...
  - 5.3.b Successful providers fetch renders one anchor per provider with the correct `href` and accessible name (resolved by `getByRole('link', { name })`).
...
- [ ] 8.3 Document in the PR description that the **`Secure` cookie over HTTP dev** issue is a separate backend concern (env-driven cookie relaxation) and is not addressed here.
- [ ] 8.4 Open a follow-up change `auth-frontend-csrf` once the backend `CsrfGuard` (Slice 5) lands — that change will wire the `X-CSRF-Token` header read from the `/api/v1/auth/me` response into non-GET calls in `server-api/base.ts`.
