...
The skill's DIAL Core `permissions` array is carried through the mapping but SHALL NOT enable any action in this change.
...
- **THEN** it offers exactly the entries it offers today, with no Skill option

---

### Requirement: Skill listing failure is surfaced without breaking the catalog

When the skills context reports a non-null `error`, `CatalogView` SHALL surface it once through the existing notification path (`useOperationNotification`) using the new i18n key `catalog.skillsLoadError`, and SHALL continue rendering every other catalog item.
...
- **Caching**: no new client cache. Listings are read once per provider mount; `refetchSkills` / `refetchPublicSkills` exist for future mutation flows and are not invoked by this change.
