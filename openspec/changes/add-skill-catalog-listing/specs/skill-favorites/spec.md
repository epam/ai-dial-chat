...
- **Request body** (`UpdateInstalledSkillDto`, new, in `apps/chat-api/src/user-config/dto/update-installed-skill.dto.ts`): a `class-validator`-validated class with `id: string` (`@IsString`, `@MinLength(1)`, `@MaxLength(2048)`, and an allowlist `@Matches` accepting `skills/{bucket}/{path}` — the same shape `parseSkillResourceUrl` accepts, so the value can never reach a path or a log line unvalidated) and `isInstalled: boolean` (`@IsBoolean`). Both fields carry `@ApiProperty` with a description and example.
