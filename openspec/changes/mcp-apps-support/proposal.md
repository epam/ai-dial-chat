...
- **Follow-up, not v1**: once DIAL Core surfaces a tool server's declared `_meta.ui.csp`/`_meta.ui.domain` (it doesn't in Phase 1), `chat-api`'s resource proxy and `mcp-app-sandbox-proxy`'s CSP builder should honor that declaration and fall back to today's fixed restrictive CSP only when it's absent — additive, never wider than the current default. Independently, add an explicit deny-by-default CORS policy (no `Access-Control-Allow-Origin`, no credentials) to `chat-api`'s MCP Apps endpoints and `mcp-app-sandbox`'s route, as defense-in-depth alongside CSP rather than a fix for an active gap (no cross-origin `fetch`/XHR against them succeeds today either way).
