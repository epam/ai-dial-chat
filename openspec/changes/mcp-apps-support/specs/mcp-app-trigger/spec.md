...
3. For each returned tool, set `McpAppToolRef.mcpToolName` to the tool's real, unprefixed name (as returned by `listMcpAppTools`) and `McpAppToolRef.toolName` to `` `${toolset.displayName}_${toolName}` `` — the former is what `resolveMcpAppToolResult`'s live re-call passes to `callTool` (the mounted app only recognizes its own real name), the latter is what `findMcpAppForMessage`/`resolveMcpAppToolCallSeed`'s name-matching (`design.md` D8) compares against the message's tool-call name. Passing the re-prefixed `toolName` to the app instead of `mcpToolName` was a confirmed bug (design.md D9): the app is addressed by its real name, and a correlation-name mismatch silently starves it of data.
...
#### Scenario: The live re-call receives the tool's real name, not the correlation name

- **WHEN** `resolveMcpAppToolResult` live-calls for a `McpAppToolRef` discovered indirectly, with `toolName: 'weather_get_weather'` and `mcpToolName: 'get_weather'`
- **THEN** the `callTool` invocation uses `mcpToolName: 'get_weather'` — the mounted app only recognizes its own real name (the correlation name is never sent to the app; `McpAppCanvasContent` carries no tool-name field at all, see `mcp-app-canvas`)
...
**Revised four times** (supersedes the original `StageItem`-hosted action, the intermediate `MessageActions`-bar placement, the `Stage`-keyed lookup, and the unconditional-auto-open posture — see `design.md` D5, fourth revision / D11). Neither `libs/conversation-stages` (`StageItem`/`StagesPanel`/`CollapsedGroup`) nor `libs/conversation-messages` (`MessageActionsProps`/`MessageActions.tsx`) carry any MCP-Apps-related field. The component is `libs/mcp-apps/src/components/McpAppInlinePreview/McpAppInlinePreview.tsx`, rendered by `apps/chat/src/components/ConversationView/ConversationMessageItem.tsx` inside the message's `afterContent`, immediately below the `CollapsedGroup` stages block.

`ConversationMessageItem.tsx` SHALL compute `mcpAppMatch = findMcpAppForMessage(msg, mcpAppTools)` and, when both `mcpAppMatch` and an `onOpenApp` callback are available, render `McpAppInlinePreview` with `match={mcpAppMatch}`, `toolCall={resolveMcpAppToolCallSeed(msg, mcpAppMatch.toolName)}`, the shared `cache` (`McpAppResponseCache`, see the response-cache requirement below), `cacheKey={mcpAppCanvasKey(index)}`, `onExpand={() => onOpenApp(mcpAppMatch, mcpAppKey, toolCall)}`, `isOpenedInCanvas={isMcpAppOpenedInCanvas}`, and `openedInCanvasLabel`. **Revised**: `McpAppInlinePreview` is now always rendered — `isMcpAppOpenedInCanvas` no longer swaps the whole component out for a separate placeholder box. Instead, when the message's canvas *is* currently open, the header (tool name, appended app name/version) stays visible and only the body area shows `openedInCanvasLabel` centered in place of the mounted app, so the message and the full-width canvas never mount two live instances of the same app at once.
...
- A bordered box spanning the full available width, sized to the mounted app's actual content height (not fixed-height), with a header strip above the app — styled like the code block header in `libs/chat-shared`'s Markdown renderer (`MarkdownCodeBlock`: bordered `min-h-10` header with `border-b`, small `GhostIconButton`s in a `flex gap-1` group, `role="toolbar"` with an accessible name). **Revised** (tasks.md item 18.2): the header is `justify-between` — the start side shows `match.mcpToolName` (`dial-tiny-lead-semi-text`, `text-secondary`, the whole header text wrapped in a single `EllipsisTooltip`), and the end side carries a reload button (`IconRefresh`) and an expand-to-canvas button (`IconArrowsMaximize`, calling `onExpand`). The header sits outside the app's content so it can never be overlapped by whatever the app draws. **Further revised**: once the mounted app completes its `ui/initialize` handshake, its declared name/version is appended after the tool name, in this order: `match.mcpToolName`, a small vertical divider (a bordered `span`, not a text character), app name (`dial-tiny-lead-semi-text`, same size as the tool name), app version (`dial-caption-text`, smaller). All of it shares `text-secondary` via the outer `EllipsisTooltip`. This state resets whenever `cacheKey` changes so a stale name/version never lingers onto a different message's app while it loads.
- While `isOpenedInCanvas` is `true` (the message's canvas is currently open), the header above still renders unchanged, but the body area shows `openedInCanvasLabel` centered (`role="status" aria-live="polite"`, `text-secondary`) instead of the loading spinner/error/`McpAppCanvasRenderer` below — the app is never mounted twice at once.
- A loading spinner, an error message, or the mounted `McpAppCanvasRenderer` inside that box, depending on fetch/resolve status.
- An app-initiated `ui/request-display-mode` request for `'fullscreen'` expands into the full-width canvas via the same `onExpand` callback the expand button uses; any other requested mode (including `'pip'` and `'inline'`) keeps the preview, and the app is answered with `'inline'` (see the display-mode requirement in `mcp-app-canvas`). The preview passes this mapping to `useMcpAppInlinePreview`'s optional `onRequestDisplayMode` parameter — the hook itself has no opinion on surface switching.
...
2. Call `closePanel()` and `closeSourcesPanel()` synchronously (same mutual-exclusivity contract as every other canvas trigger in the `canvas` capability), then `openCanvasLoading(title, canvasKey)`.

**Revised** (tasks.md item 18.2): `title` is `match.mcpToolName` — the matched tool's real name — not the fixed `AttachmentCanvasI18nKeys.McpAppTitle` string used previously. The panel header (and the inline preview's own header strip, see the requirement above) now shows which tool the mounted app corresponds to instead of a generic "MCP App" label. `UseOpenMcpAppCanvasLabels`/`McpAppCanvasLabels` no longer carries a `title` field — it was the sole reader of that translation key, which is removed. **Further revised**: once the mounted app completes its `ui/initialize` handshake (`McpAppCanvasRenderer`'s `onAppInfo`, see `mcp-app-canvas`), `AttachmentCanvas` prepends the app's declared name/version before `fileName` in the panel title, same order and sizing as the inline preview's header: app name (inherits the panel title's own size), app version (`dial-caption-text text-secondary`, smaller), a small vertical divider (not a text character, `text-secondary`), then `fileName` (same size as the app name).
...
4. On success, build an `McpAppCanvasContent` with `html`, `sandboxUrl: mcpAppSandboxUrl`, `toolInput: toolCall?.toolInput`, `toolResult` (from step 3), `hostContext` (the `'fullscreen'` `McpUiHostContext` above), `onToolCall` bound to a `apps/chat/src/server-api/mcp-apps.ts` wrapper that POSTs to the tool-call-forwarding endpoint with `match.toolsetId`, `onRequestDisplayMode` bound to a callback that closes the canvas (`closeCanvas()`) and answers `'inline'` when the requested mode is `'inline'`, and answers `'fullscreen'` (keeping the canvas) otherwise — see the display-mode requirement in `mcp-app-canvas` — and `onReload` bound to a closure that invalidates the cache entry (`cache.invalidate(canvasKey)`) and re-invokes `openMcpAppCanvas` with `forceReload: true`. Call `openCanvas(content, title, canvasKey)` and return `true`.
...
`useMcpAppHostContext(displayMode: 'inline' | 'fullscreen')` (`apps/chat/src/hooks/attachment/useMcpAppHostContext.ts`) is the single source of the `McpUiHostContext` (imported from `@modelcontextprotocol/ext-apps/app-bridge`) both the canvas and the inline preview build from, populated as follows — this is the UI context delivered to the View during `ui/initialize`:
   ...
   - `userAgent`: `mcpAppUserAgent ?? navigator.userAgent` — admin-configurable host identifier (`MCP_APP_USER_AGENT` env var, from `AppConfigContext.config.mcpAppUserAgent`); **further revised** (`design.md` D21): falls back to the browser's own `navigator.userAgent` when unset, not a fixed `'ai-dial-chat'` string.
   ...
   - `availableDisplayModes`: the display modes the host can switch the app between via `ui/request-display-mode` — defaults to `['inline', 'fullscreen']` (the compact inline preview and the full-width canvas), host-overridable through `McpAppHostContextParams.availableDisplayModes`. A well-behaved app checks this before requesting a mode switch.
...
**Added** (`design.md` D21): `apps/chat/src/hooks/attachment/useMcpAppHostAdapter.ts` passes `{ name: 'ai-dial-chat', version: config.appVersion }` as the fifth `hostInfo` argument to `@epam/ai-dial-chat-hooks/mcp-apps`'s `useMcpAppHostAdapter` — this is the host's own identity, distinct from `hostContext`, delivered during the same `ui/initialize` handshake (the second positional argument to `@mcp-ui/client`'s `AppBridge` constructor). It flows through `McpAppHostAdapter.hostInfo` → `McpAppCanvasContent.hostInfo` (both the canvas's and the inline preview's built content) → `McpAppCanvasRenderer`/`useMcpAppBridge`, which falls back to a generic `{ name: 'MCP-UI Host', version: '1.0.0' }` identity when a caller omits it (see `mcp-app-canvas`).
