...
- A bordered box spanning the full available width, sized to the mounted app's actual content height (not fixed-height), with a header strip above the app — styled like the code block header in `libs/chat-shared`'s Markdown renderer (`MarkdownCodeBlock`: bordered `min-h-10` header with `border-b`, small `GhostIconButton`s in a `flex gap-1` group, `role="toolbar"` with an accessible name) — carrying a reload button (`IconRefresh`) and an expand-to-canvas button (`IconArrowsMaximize`, calling `onExpand`). The header sits outside the app's content so it can never be overlapped by whatever the app draws.
