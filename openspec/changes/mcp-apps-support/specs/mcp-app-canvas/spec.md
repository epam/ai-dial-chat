...
- The `AppBridge` is created inside a mount-scoped `useEffect` (not during render), with capabilities `{ openLinks: {} }` — the same default `AppRenderer` used — and its `hostContext` option seeded from `content.hostContext`. The bridge is `close()`d on unmount (a close failure is `console.error`-logged, not thrown). **Further revised** (`design.md` D21): host info is `content.hostInfo ?? { name: 'MCP-UI Host', version: '1.0.0' }` — a lib must not hardcode the embedding app's own name/version (library isolation), so the app layer supplies its real identity (`{ name: 'ai-dial-chat', version: appVersion }`) through `content.hostInfo`, and the fixed default only applies when a caller omits it.
- The `AppFrame`'s `sandbox` prop is `{ url: new URL(content.sandboxUrl) }`, memoized on `sandboxUrl` — `AppFrame` re-creates its sandbox iframe whenever the URL object's identity changes, so a literal `new URL(...)` per render would churn the iframe on every host re-render. The `html` prop skips `@mcp-ui/client`'s own resource-fetching path entirely — the renderer never issues its own network request for the resource. `hostContext` is forwarded to the bridge via `setHostContext` whenever the (resize-merged) value changes, so a live resize reaches the app as `ui/notifications/host-context-changed`. **Further revised** (`design.md` D20): the app's very first `hostContext` — returned synchronously in the `ui/initialize` response, before any resize notification can fire — also carries `containerDimensions`, measured via `containerRef.current.getBoundingClientRect()` inside `useMcpAppBridge`'s bridge-creation effect. Without this, an app that only reads `containerDimensions` once at init (rather than on every later notification) saw `undefined` and could lay out at zero width/height, even though the iframe's own CSS box was already sized correctly.
...
- Display a loading state until the frame signals readiness (`onInitialized`/`onSizeChanged` callbacks) or an error (`onError` for an initialization/channel-level failure — individual tool-call failures are relayed to the app as normal JSON-RPC errors via `onToolCall`'s return value and do NOT put the renderer itself into an error state). **Further revised** (`design.md` D19): the component's root div carries `min-h-[200px]` alongside `h-full`, so this loading overlay (`absolute inset-0`) has a real area to fill even when `h-full` has no definite ancestor height to resolve against — the case in `McpAppInlinePreview`, whose wrapper sets only `min-height`. Without the floor, the overlay collapses to zero height and the preview appears to pop in from nothing once the app's iframe reports its real size, instead of showing a spinner throughout.
