...
`CatalogView.tsx` SHALL be composed from six focused hooks under `apps/chat/src/hooks/`, each owning exactly one concern's state, memoized derivations, and `server-api` calls, with no concern's state or handler duplicated across two hooks:

- `useCatalogItems` SHALL own: `quickAppSchemaId`, `quickAppDeploymentIds`, the label memos (`promptOverviewLabels`, `skillOverviewLabels`, `deploymentLimitsLabels`), `catalogDetailsApi`, `combinedSkills`, the `useCatalogItemDetails` call (`onFetchDetails`, `onLoadContentFile`, `onLoadSkillDetailsFile`), `catalogItems`, `visibleCatalogItems`, `reconciledFilterTopics`, `availableTabIds`, and `favorites`.
- `useCatalogToolsetCredentials` SHALL own: `getLevelStatus`, `showLoginSuccess`, `showLogoutSuccess`, `handleLogin`, `handleLogout` (including its `useToolsetLogin` call).
- `useCatalogPublishing` SHALL own: `getPublishHistory`, `handlePublish`, `handleUnpublish`, `handlePublishSuccess`, `handlePublishError`, `handleFetchExistingRules`, `isPublishVisible`.
- `useCatalogSharing` SHALL own: `isShareVisible`, `isUnshareVisible`, `isRevokeShareVisible`, `handleUnshare`, `handleRevokeShare`, `handleFetchRecipientsCount`.
- `useCatalogItemActions` SHALL own: `fetchPromptDto`, `handleUseInChat`, `handleCardSelect`, `handleDownload`, `isDownloadVisible`, `isPrimaryActionVisible`, `onToggleFavorite`, `renderContentFilePreview`.
- `useCatalogEditNavigation` SHALL own: `buildEditorUrl`, `handleEdit`, `handleDelete`, `createOptions`.
