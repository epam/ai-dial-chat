...
### Requirement: Command-menu popup reopens when backspacing back to the bare trigger

The slash-command popup (opened by typing the trigger character, e.g. `/`, into an empty textarea) SHALL reopen when the textarea's value becomes exactly the bare trigger character again after having been dismissed (Escape or an outside click) while more characters followed the trigger — for example: typing `/sdf`, dismissing the popup, then backspacing `/sdf` → `/sd` → `/s` → `/`. The popup SHALL NOT reopen at any intermediate value with characters still following the trigger (e.g. it does not reopen at `/sd` or `/s` during that same backspacing sequence), and SHALL NOT reopen merely because the value still matches the trigger-plus-query shape without actually returning to the bare trigger.
