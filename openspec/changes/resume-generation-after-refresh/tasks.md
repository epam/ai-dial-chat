...
- [x] 3.2 Guard against calling `resumeIfAwaitingGeneration` more than once for the same path from React StrictMode's double-invoke (mirror the existing `autoStartedPathsRef` pattern used for the `ContinueLastUser` auto-start branch).
- [x] 3.3 Confirm no changes are needed in `useConversationHandlers.ts`, `ConversationView.tsx`, `ConversationMessageItem.tsx`, or `build-message-actions.ts` — their existing `isStreaming`/`isAssistantTyping` consumption should already produce the typing indicator and guarded Regenerate/edit no-ops once `streamingPaths` includes the resumed path. Add a one-line comment only if a non-obvious interaction is found.
