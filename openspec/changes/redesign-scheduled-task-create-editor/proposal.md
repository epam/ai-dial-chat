...
- **Configuration** column: replace the prompt `Textarea` with `DialMarkdownEditor` from `@epam/ai-dial-ui-kit`, bound to the existing `values.prompt` field; import the ui-kit's markdown editor CSS once at the `apps/chat` entry.
...
- `apps/chat` entry (main/layout) — one-time import of `DialMarkdownEditor` CSS.
