...
### Wrap `GhostIconButton` in `<Link>` rather than replacing it

React Router's `<Link to={path}>` renders a native `<a href="...">`. Wrapping the existing `GhostIconButton` inside it is the minimal-change approach:
...
- The `GhostIconButton` keeps all its existing props (`icon`, `aria-label`, `aria-current`, `tooltipProps`, `className`).

**Alternatives considered:**

| Alternative | Why rejected |
|---|---|
| Pass `href` directly to `GhostIconButton` | The ui-kit component does not expose an `href` prop — it always renders a `<button>`. |
| Replace `GhostIconButton` with a custom `<a>` styled to look identical | More styling work; loses ui-kit hover/focus/press states and tooltip integration. |
...
- **Focus order**: `<a>` wrapping a `<button>` creates two focusable elements. Mitigation: add `tabIndex={-1}` to the `GhostIconButton` so only the `<a>` is in the tab order. The button will still respond to pointer events and the `<a>` receives keyboard focus.
