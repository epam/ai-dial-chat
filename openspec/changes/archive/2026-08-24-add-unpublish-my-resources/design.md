...
The `503` is attributable and fixed, and it was never Core being unavailable. Verified against a live Core during this change, three defects sat in our own history path:

1. **The `503` itself.** The SDK types `getPublications` as `Publication[]`, but Core answers with a `{ publications: [...] }` envelope. `.filter` on that object threw `TypeError: (result.data ?? []).filter is not a function`, and `handleDialFetchError` reported the throw as "DIAL Core is currently unavailable". `toPublicationList` (`publish/publication.util.ts`) now accepts both shapes. **This is what GH #7897 actually was** — our bug, wearing Core's error message, which is why it was read as a Core outage and worked around with stubs.
2. **The list scope.** `PublishService` derived it from `entityId` (`getPublicationsListScope(getResourceBucket(sourceUrl))`), so a bare deployment id produced `publications/undefined/` and a public resource produced the admin-only `publications/public/`. It now scopes by the session `bucket`, as `ConversationPublishService` already did.
3. **The resource filter.** Core's list response carries publication *metadata* only — `url`, `status`, `targetFolder`, `createdAt`, `author` — and **no `resources` array**. Filtering the list on `resources[].sourceUrl` therefore matched nothing: a live Core returned 60 publications and history still came back empty, which hid `Unpublish` on an entity that was demonstrably published. `resolvePublicationsForSource` re-reads each `APPROVED` candidate through `getPublication` (Core exposes no per-resource filter on the list call) and matches there. Only `APPROVED` publications are candidates — a `PENDING` request has not created a published copy and a `REJECTED` one never will — which is also what keeps the lookups proportional to real publications; they are batched, and a failed lookup drops that one publication rather than the whole history.

So the stubs worked around defects that no longer exist — which is the premise for turning them back on.
...
- Does the conversation publish-history endpoint work against the current Core deployment? The catalog one does, after the three fixes in D6, verified live during this change; the conversation service shares `toPublicationList`, `resolvePublicationsForSource` and the session-bucket scope but has not been driven end to end. The answer decides whether catalog and conversation unpublish are usable on day one or only after a Core-side fix, and — for the catalog panel — whether restoring the fetch re-exposes a history error state to users.
