...
**Styling.** `OoxmlContent.module.scss` SHALL set only the viewer and overlay backgrounds (`--bg-layer-raised`) and read the canvas's existing status/error variables — `var(--ac-status-text, var(--text-secondary, …))` and `var(--ac-error-icon, var(--text-error, …))` — so Office previews theme with the rest of the panel. Layout stays in Tailwind. No new `*Colors` field is introduced.
