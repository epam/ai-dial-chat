...
or while the deployments context is loading. During either loading state, `UsageTab` SHALL render a
visible, localized loading indicator and withhold the aggregate cards, Model limits table, and empty
state. The section SHALL render nothing (or an explicit empty state) when `usage` is `undefined` or
`usage.deployments` is absent/empty after both loading states complete.

#### Scenario: Usage request renders a loading indicator
- **WHEN** `useUsageData()` reports `isLoading: true`
- **THEN** the page renders a visible localized loading indicator and does not render cards, the
  Model limits table, or its empty state

#### Scenario: Deployments request renders a loading indicator
- **WHEN** usage has loaded but the deployments context reports `isLoading: true`
- **THEN** the page renders a visible localized loading indicator and does not render cards, the
  Model limits table, or its empty state
