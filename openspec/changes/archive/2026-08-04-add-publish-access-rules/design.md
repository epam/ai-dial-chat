...
- **[Risk]** A large operator-configured `publicationFilterSources` list could make the plain `DialSelect` dropdown unwieldy. → **Mitigation**: `DialSelect` already supports `searchable`; the source picker enables it, with matched text rendered via the shared `Highlight` component per `.claude/rules/search-results-highlight.md`.
