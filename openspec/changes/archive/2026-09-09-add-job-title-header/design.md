...
**Restore Keycloak's UserInfo source.** On `development-old`,
`apps/chat/src/utils/auth/auth-providers.ts:220` explicitly requests UserInfo,
and `auth-callbacks.ts:378` copies `options.profile.job_title`. The initial port
only read ID-token claims, losing job titles issued exclusively through
UserInfo. When the ID-token value is absent, empty, or not a string, the callback
uses the existing OIDC client's discovered UserInfo endpoint for Keycloak only,
provided an access token and endpoint are available. It checks `sub` against the
validated ID token before copying only `job_title`; no roles or unrelated
profile fields are merged. An existing non-empty ID-token value takes precedence.
Errors and subject mismatches leave login available without the optional value,
and produce a fixed warning without the provider error or response payload.
The request uses openid-client's existing HTTP timeout. This avoids requiring
realm mapper changes or adding profile requests for every provider.
...
- **[`job_title` absent from all supported sources]** → Keycloak falls back to UserInfo; if no usable value is available there either, `buildJobTitleHeaders` omits the header. Other providers use the ID token only.
