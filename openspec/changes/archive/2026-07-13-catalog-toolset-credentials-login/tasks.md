...
- [x] 7.3.7 `CredentialsSection.tsx`: branch on `isManageableByAdmin` to render two `DialAccordion` sections (`LevelForm` sub-component per level) vs. a single section at the resolved level; show the API-key hint via `apiKeyFieldHint`.
