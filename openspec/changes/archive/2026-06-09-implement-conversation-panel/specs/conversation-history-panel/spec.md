...
> **Implementation note:** `libs/conversation-panel` imports `SidebarPanel`, `SearchInput`, and `SidebarOrientation` from `@epam/ai-dial-sidebar` to use as the panel shell. Design decision 1 (no cross-lib sidebar dependency) was revised during implementation — sharing the `SidebarPanel` shell avoids duplicating panel layout and CSS custom-property logic across libs.
...
> **Implementation note:** The desktop toggle button lives in `apps/chat/src/components/Header/Header.tsx` via `isHistoryPanelOpen` and `onHistoryPanelToggle` props. The panel width when open is `w-[325px]` (not `w-[320px]`).
...
> **Implementation note:** Specification originally described only 2 groups (Pinned + My chats). Shared and Organization sections were added during implementation to match the four `ConversationSource` values.
...
> **Implementation note:** Row-level actions (pin/unpin, rename, delete) were added after the initial spec; they are wired in `ConversationPanelView` where `ConversationsContext` supplies the mutation methods.
...
> **Implementation note:** The original spec described an `onBackdropClick` backdrop overlay. This prop was not implemented; mobile close is handled exclusively via the close button inside the panel header (via `onToggle` → `SidebarPanel.onClose`).
...
### Requirement: Panel collapses with aria-expanded

**Reason:** The `SidebarPanel` shell uses `aria-hidden` on the inner `<aside>` element instead of `aria-expanded` on the outer wrapper. Accessibility state is communicated via `aria-hidden={!isOpen}`.

### Requirement: Backdrop overlay on mobile (onBackdropClick)

**Reason:** The backdrop overlay approach was dropped during implementation. Mobile close is handled by a close button inside the panel header, wired via `onToggle` prop on `ConversationPanel` (delegated to `SidebarPanel.onClose`).
