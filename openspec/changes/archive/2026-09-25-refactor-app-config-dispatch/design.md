...
- [A default array is shared between cached responses, so mutation leaks across
  callers] → The factory and wrong-shape fallbacks return fresh arrays and objects, and
  a mapper test checks that. Provider-returned values stay shared by reference, as
  they are today. That includes registry `defaultValue` arrays and objects that reach
  the mapper through `value ?? def.defaultValue`, which is the common path for unset
  keys. This is a documented, unchanged trade-off. The service tests therefore assert
  per-caller value isolation, not reference inequality.
