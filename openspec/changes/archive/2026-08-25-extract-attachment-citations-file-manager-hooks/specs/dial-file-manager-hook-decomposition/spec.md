...
`useDialFileManager` (exported from `@epam/ai-dial-chat-hooks`) SHALL be composed from five focused sub-hooks, each owning exactly one concern's state, effects, and `DialFilesApi` operation-port calls, with no concern's state or handlers duplicated across two sub-hooks:
