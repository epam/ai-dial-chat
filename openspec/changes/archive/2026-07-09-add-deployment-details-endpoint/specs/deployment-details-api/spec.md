...
- SHALL resolve the deployment's type from the `deployment` id prefix, mirroring the `toolsets/`/`applications/` prefix convention already relied on by the frontend (`apps/chat/src/utils/map-deployment-to-catalog-item.ts`'s `TOOLSETS_PREFIX`/`APPLICATIONS_PREFIX`), rather than calling `listDeployments` — avoids an expensive full-catalog fetch just to classify one id:
  ...
  - otherwise (ambiguous — root-level applications don't always carry the `applications/` prefix) → call `getModel` first; if that returns a 404, fall back to `getApplication` for the same id before giving up.
- SHALL respond 404 when neither the direct call nor (for ambiguous ids) the `getApplication` fallback finds the deployment.
...
- MUST NOT forward `function.env`, `function.source_folder`, `function.target_folder`, `auth_settings` credentials/secrets, `editor_url`, or raw `reference` fields.
...
- **THEN** the endpoint responds 200 with `DeploymentDetailsDto` whose `type` is `'toolset'` and `toolsetDetails` is populated from `getToolset`, with `authSettings` reduced to `{ authenticationType }` only (no client credentials)
...
- **THEN** the direct call (or, for an ambiguous unprefixed id, both the `getModel` call and the `getApplication` fallback) returns 404 and the endpoint responds 404
...
- `toolsetDetails?: ToolsetDetailsDto` — present only when `type === 'toolset'`:
  - `transport?: string`
  - `allowedTools?: string[]`
  - `allToolNames?: string[]` — from `GET /v1/toolset/{id}/tools` (`getAllToolSetTools`), a best-effort supplementary call: a failure or non-2xx response is logged and omits this field without failing the whole request
  - `authSettings?: ToolsetAuthSettingsDto` — `{ authenticationType?, globalAuthStatus?, appLevelAuthStatus?, userLevelAuthStatus?, scopesSupported?, authorizationEndpoint?, tokenEndpoint?, apiKeyHeader? }`; never the raw `auth_settings` object (client credentials excluded)
  - `owner?: string`
  - `features?: DeploymentFeaturesDetailsDto`
  - `createdAt?: number`
...
- **WHEN** the underlying DIAL Core response for an application includes `function.env` or a toolset includes `auth_settings.client_secret`-shaped fields
- **THEN** the mapped `DeploymentDetailsDto` does not contain those values anywhere in its JSON representation
