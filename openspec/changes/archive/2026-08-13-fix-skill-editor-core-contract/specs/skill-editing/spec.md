...
When the edit or create form has unsaved changes (`onDirtyChange(true)` most recently reported by `libs/skill-editor`), the page SHALL confirm before: activating Cancel, activating the page's Back control, or navigating away via any other in-app route change that the page can intercept. A confirmed navigation proceeds; a declined one leaves the user on the page with their edits intact.
