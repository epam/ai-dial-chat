...
Surfaces DIAL Core WRITE-permission share grants on applications and toolsets as a `canEdit`/`can_edit` field, so a user who was shared a resource with edit access (not just view) sees the catalog's Edit action for it, not only for resources they own.
