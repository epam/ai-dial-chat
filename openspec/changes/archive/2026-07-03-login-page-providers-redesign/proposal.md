...
- **Full-screen background**: Two static images (`768_login.svg`, `1920_login.svg`), served via a `<picture>` element with a `(min-width: 1920px)` media query; hidden on mobile where the card is transparent. No light/dark theme variants — a single image set is used regardless of theme.
...
- **Added**: `apps/chat/public/1920_login.svg` — large-screen background image
- **Added**: `apps/chat/public/768_login.svg` — mid-size background image
