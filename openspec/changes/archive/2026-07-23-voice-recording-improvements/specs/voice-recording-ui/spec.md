...
`ConversationInput` SHALL render a ghost icon button (UI kit `DialGhostIconButton`, 40 px outer / 24 px icon) on the right side of the action bar when `isAudioMessageSupported` is `true` **and the message text field is empty**. The button SHALL be hidden when `isAudioMessageSupported` is `false`, not provided, or when the text field contains non-whitespace characters.
