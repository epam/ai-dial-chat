...
- Settings modal (`DialPopup`) lets the user pick a theme via `Select`; selection is immediate (no Save button needed — `setTheme` is synchronous/localStorage-backed)
...
### D5: `Select` for theme picker (not radio buttons)
The themes list is dynamic (loaded from the backend via `useTheme().themes`). `Select` handles a variable-length options list naturally. `DialRadioGroup` is better suited for a small, static, labeled set.

## Risks / Trade-offs

- **Theme list loading race** → `useTheme().isLoading` is `true` while themes load. The `Select` should be `disabled` when `isLoading` is true to prevent interaction before options are ready.
