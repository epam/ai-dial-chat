...
The `Select` SHALL be `disabled` when `useTheme().isLoading` is `true`.
...
- **THEN** the theme `Select` shows the currently active theme as the pending selection

#### Scenario: Theme selection is staged, not applied immediately
- **WHEN** the user selects a different theme in the `Select`
...
- **THEN** the `Select` is rendered in the disabled state
