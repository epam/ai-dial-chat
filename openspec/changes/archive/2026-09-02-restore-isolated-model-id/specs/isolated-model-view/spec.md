...
**State ownership:** `IsolatedModelViewContext` (new). Reads `useLocation()` and `useDeployments()` (existing state owners). Drives `UiFeaturesContext.applyIsolatedViewOverride` (see the `ui-feature-toggles` delta spec); introduces no other new persisted state.
