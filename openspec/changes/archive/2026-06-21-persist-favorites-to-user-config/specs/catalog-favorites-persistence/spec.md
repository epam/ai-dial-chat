...
`UpdateInstalledDto.id` in `apps/chat-api/src/user-config/dto/update-installed.dto.ts` SHALL be validated with `@Matches(/^\S+$/)` instead of the previous `/^[\w\-./@]+$/` allowlist.

Real deployment IDs can contain `:` and other characters not in the original allowlist (e.g. `anthropic.claude-opus-4:0`). The only structural constraint on a deployment ID is that it contains no whitespace.
