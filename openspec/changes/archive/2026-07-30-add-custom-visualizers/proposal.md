...
- Add host-owned visualizer models (`CustomVisualizer`, layout/data payloads) to `libs/chat-shared/src/models/custom-visualizer.ts`.
- Consume the published stable npm packages `@epam/ai-dial-visualizer-connector` and `@epam/ai-dial-shared` for the host-side iframe manager (no workspace `libs/visualizer-connector`). Third-party visualizer authors continue to use published `@epam/ai-dial-chat-visualizer-connector` from the legacy Chat line — this monorepo does not vendor or republish that package.
- Extend `libs/attachment-canvas` with a new `AttachmentContentType.Visualizer` variant, a `VisualizerCanvasContent` model, and a `VisualizerCanvasRenderer` internal component that instantiates the published `VisualizerConnector`, performs the `READY`/`READY_TO_INTERACT` handshake, and sends `SEND_VISUALIZE_DATA` (via `@epam/ai-dial-shared`'s `sendVisualizeData` enum member) with a `CustomVisualizerDataLayout` payload (`themeId` plus the registry entry's optional size hints).
...
- `custom-visualizers`: registry of MIME → visualizer URL entries; host wiring that opens the canvas with a visualizer renderer when an attachment's MIME matches the registry; consumption of the published `@epam/ai-dial-visualizer-connector` postMessage/iframe manager (not an in-repo port of that package).
...
- **Code (new):** `libs/chat-shared/src/models/custom-visualizer.ts`, `libs/attachment-canvas/src/components/VisualizerCanvasRenderer/`.
- **Code (modified):** `libs/attachment-canvas` (`types/attachment-canvas.ts`, `models/attachment-canvas.ts`, `components/AttachmentCanvas/AttachmentCanvas.tsx`, `package.json`, `vite.config.mts`); root `package.json` / lockfile; `apps/chat-api/src/config/environment.config.ts`; `apps/chat-api/src/app-config/config-registry/config-registry.constants.ts`; `apps/chat-api/src/app-config/dto/` (new `CustomVisualizerDto`, extended `AppConfigResponseDto`); `apps/chat/src/context/AppConfigContext.tsx`; `apps/chat/src/hooks/attachment/useOpenAttachmentCanvas.ts`. `apps/chat/src/hooks/attachment/useAttachmentAction.ts` is NOT modified (see `design.md` D4).
...
- **Dependencies:** add published `@epam/ai-dial-visualizer-connector` and `@epam/ai-dial-shared` (stable npm, currently `0.48.0`) as workspace root dependencies and as `attachment-canvas` peerDependencies. No workspace copies of those connector libs.
