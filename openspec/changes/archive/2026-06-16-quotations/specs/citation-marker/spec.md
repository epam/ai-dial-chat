...
`apps/chat/src/utils/groupAnnotationsBySource.ts` SHALL export `groupAnnotationsBySource(annotations: Annotation[]): AnnotationGroup[]` where `AnnotationGroup` is:
```ts
...
`apps/chat/src/components/Citations/CitationMarker/CitationMarker.tsx` SHALL render a UI kit `Button` (variant chip/tag, size small) with:
...
- Logical margin `ms-1` to separate it from the preceding text
...
**RTL**: uses `ms-1` (logical); the button itself is direction-agnostic (text content only, no directional icon).
...
- If the resolved insertion point falls inside a markdown inline element (e.g., inside `**bold**`), the marker SHALL be inserted immediately after that element's closing boundary.
...
**RTL**: the injected markers use `ms-1`; no additional RTL handling needed.
**Accessibility**: `role="group"` on the text+markers container with `aria-label` for screen readers is RECOMMENDED but not required in this slice.
**Memoisation**: the injected tree SHALL be memoised with `useMemo` keyed on message text and annotation list reference to avoid re-injection on unrelated re-renders.
