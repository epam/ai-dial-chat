...
`apps/chat/src/components/Citations/CitationCard/CitationCard.tsx` SHALL render a popover panel anchored to a trigger element with:
...
- **WHEN** `CitationCard` is rendered with a group of three annotations and `activeIndex={0}`
...
When the "Preview" button is clicked in `CitationCard`, the app SHALL invoke the existing attachment-preview flow (the same handler used when clicking an `AttachmentCard` in the assistant bubble) with the `Annotation.body.source.attachment` as the attachment argument.
