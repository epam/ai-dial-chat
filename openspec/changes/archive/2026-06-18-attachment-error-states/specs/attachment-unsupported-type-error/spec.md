...
A file is invalid when none of the allowed MIME entries match: exact equality (`application/pdf`) OR wildcard prefix match (`image/*` matches `image/jpeg`). When `inputAttachmentTypes` is empty or undefined, the attachment button is already hidden and this path is unreachable.
