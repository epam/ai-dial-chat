...
- **message**: i18n key `attachments.networkError.message` → "Some files were not uploaded because your internet connection was lost. Please try again: {{files}}", where `{{files}}` is a comma-separated list of the original attachment filenames (with extension).
...
- `attachments.networkError.message` (supports `{{files}}` interpolation)
