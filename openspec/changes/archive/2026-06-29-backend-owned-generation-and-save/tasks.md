## 1. Backend â€” DTO changes (apps/chat-api)
...
- [x] 1.4 Run `npm exec nx run chat-api:build` â€” no errors

## 2. Backend â€” Generation registry (apps/chat-api)
...
- [x] 2.2 Key format: `` `${sessionId}::${path}` `` â€” one active generation per session+path; `generationId` is stored in the entry for validation on stop
...
- [x] 2.5 Run `npm exec nx run chat-api:build` â€” no errors

## 3. Backend â€” Server-side chunk assembler (apps/chat-api)
...
- [x] 3.5 Run `npm exec nx run chat-api:test` â€” all new tests pass

## 4. Backend â€” Conversation history builder (apps/chat-api)
...
- [x] 4.8 Run `npm exec nx run chat-api:test` â€” all new tests pass

## 5. Backend â€” Refactor `ConversationService.streamCompletion` (apps/chat-api)
...
- [x] 5.10 Remove the existing `return result.response.body` pattern â€” the method now writes directly to `res` and returns `Promise<void>`
- [x] 5.11 Inject `ConversationGenerationService` into `ConversationService` constructor

## 6. Backend â€” Stop endpoint (apps/chat-api)
...
- [x] 6.4 Run `npm exec nx run chat-api:lint` and `npm exec nx run chat-api:build` â€” no errors

## 7. Backend â€” Controller wiring (apps/chat-api)
...
- [x] 7.3 Remove SSE header setup from controller â€” move it into `ConversationService.streamCompletion` (or a shared helper) so the service controls response lifecycle
- [x] 7.4 Run `npm exec nx run chat-api:build` â€” no errors

## 8. Backend â€” OpenAPI regeneration

- [x] 8.1 Run `npm run openapi` to regenerate the OpenAPI spec from the updated backend
- [x] 8.2 Run `npm run openapi:check` â€” no drift
- [x] 8.3 Run `npm exec nx run chat-api-client:build` â€” no errors

## 9. Frontend â€” `GenerationContext` (apps/chat)
...
- [x] 9.3 `startGeneration(path, generationId): AbortController` â€” creates `AbortController`, stores entry keyed by `path`, returns controller
- [x] 9.4 `stopGeneration(path, generationId): void` â€” calls `abortController.abort()` locally (does NOT call backend; caller is responsible for calling `stopCompletion` API)
- [x] 9.5 `completeGeneration(path, generationId): void` â€” marks status `done`
...
- [x] 9.8 Run `npm exec nx run chat:build` â€” no errors

## 10. Frontend â€” `chat-stream.api.ts` changes (apps/chat)
...
- [x] 10.4 Run `npm exec nx run chat:build` â€” no errors

## 11. Frontend â€” `useConversationStream.ts` changes (apps/chat)

- [x] 11.1 Accept `generationId` as parameter on `startStream`
- [x] 11.2 In `onChunk`: add guard â€” `if (activeGenerationId !== generationId) return` before applying chunk to state
...
- [x] 11.8 Run `npm exec nx run chat:build` â€” no errors

## 12. Frontend â€” `useConversationHandlers.ts` changes (apps/chat)
...
- [x] 12.6 Run `npm exec nx run chat:build` â€” no errors

## 13. Backend â€” Integration tests (apps/chat-api)
...
- [x] 13.5 Run `npm exec nx run chat-api:test` â€” all pass

## 14. Frontend â€” Unit tests (apps/chat)
...
- [x] 14.5 Run `npm exec nx run chat:test` â€” all pass
