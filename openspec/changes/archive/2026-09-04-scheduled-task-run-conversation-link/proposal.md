...
- Apply the same conditional-click + unread-dot wiring to the History section inside `ConversationSourcesPanel` (the right-hand sources sidebar shown while viewing a scheduled-task conversation) — the same `ScheduledTaskRunHistoryList` component renders there too, and a user browsing a task's other runs from inside one of its conversations has the same need to jump between them.
...
- `conversation-sources-sidebar`: the History section's "row click is a no-op" requirement is superseded — a row becomes conditionally clickable (only when its run has a `conversationId`) and shows the same unread dot, clicking navigates to that run's conversation.
...
- App: `apps/chat/src/utils/map-scheduled-task-run-dto.ts`, `apps/chat/src/pages/ScheduledTaskDetailPage/ScheduledTaskDetailPage.tsx` (+ tests), `apps/chat/src/components/ConversationSourcesPanel/ConversationSourcesPanel.tsx` (+ tests), `apps/chat/src/i18n/locales/en.json` (reused key only, no new string expected).
