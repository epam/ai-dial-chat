...
- Respond `200 OK` with `DiscardSharedResponseDto { success: true }` on success.
...
- Log structured success/failure messages (e.g. `Discard shared resource started`, `Discard shared resource completed: success=true`, `Discard shared resource failed: status=<n>`) without the access token, invitation links, full resource path, or any other user data beyond a safe operation identifier.
...
**Generated-client impact**: new operation `discardSharedCatalogItem` added to `libs/chat-api-client`'s share API surface, request DTO `DiscardSharedDto { itemId: string }`, response DTO `DiscardSharedResponseDto { success: boolean }`. Frontend callers use the normal (non-`Raw`) generated method, wrapped by a thin `apps/chat/src/server-api/share.api.ts` function.
...
`libs/catalog/src/components/Details/Header/ShareButton/ShareButton.tsx` SHALL retain its internal `onUnshare?: (item: CatalogItem) => void` callback and `unshareLabel?: string` prop, while presenting the recipient-side operation as Delete. The component SHALL render:
- the existing Share action when `item.isMyApp === true` (unchanged gating, unchanged behavior), OR
- a Delete action when `item.sharedWithMe === true` and `item.isMyApp !== true`, using the direction-neutral Tabler `IconTrash` icon (no `rtl:scale-x-[-1]` mirroring needed) and calling `onUnshare?.(item)` on click, OR
...
`libs/catalog/src/components/Details/Header/Header.tsx` and `libs/catalog/src/models/catalog-props.ts` (`CatalogProps`)/`item-details-props.ts` (`DetailsPanelProps`) SHALL thread the new `onUnshare` callback through identically to the existing `onShare` prop.
...
- **THEN** the header renders the Share action and does not render a recipient-side Delete action
...
`libs/catalog/src/components/Details/DetailsPanel.tsx` SHALL present an accessible confirmation popup before invoking `onUnshare`, reusing the `DialConfirmationPopup` component (`@epam/ai-dial-ui-kit`) already used for the existing direct-logout confirmation in this file, following the same local-state pattern (`isUnshareConfirmOpen` boolean plus request/cancel/confirm handlers).

The popup SHALL:
- Open when the Delete action (previous requirement) is clicked, instead of invoking `onUnshare` immediately.
- Display a title, a description interpolating the item's display name (e.g. via an i18n `{{name}}` placeholder supplied by the app layer, not string concatenation inside the lib), a confirm label, and a cancel label — all as translated strings passed in as props/text, never hardcoded inside `libs/catalog`.
...
**i18n keys** (added to `CatalogI18nKeys` in `apps/chat/src/constants/translation-keys.ts` and `apps/chat/src/i18n/locales/en.json`):
- `catalog.details.unshare.confirmTitle` → "Delete item?"
- `catalog.details.unshare.confirmMessage` → `Delete "{{name}}" from your catalog? You'll need a new invitation to access it again.`
- `catalog.details.unshare.confirmLabel` → "Delete"
- `catalog.details.unshare.successTitle`, `catalog.details.unshare.success`, `catalog.details.unshare.error` — mirroring the existing `catalog.details.delete.*` success/error key shapes

The unshare key namespace and `onUnshare` identifiers remain unchanged because this is a presentation-only change; the operation still discards a recipient's share and does not delete the owner's resource.

**RTL / direction impact**: the confirmation popup uses only logical Tailwind/CSS properties (inherited from `DialConfirmationPopup`); `IconTrash` is symmetric/direction-neutral and SHALL NOT be mirrored with `rtl:scale-x-[-1]`. The interpolated item name is inserted via i18next placeholder substitution so bidi rendering of mixed-direction names is handled by the browser's Unicode bidi algorithm, not manual string concatenation.
...
- **THEN** a confirmation popup opens with the title "Delete item?" and a description containing "My Shared App"; `onUnshare` has not yet been called
...
### Requirement: Catalog view integration — mutate, refetch, clear stale selection, close panel
...
- Calls the new frontend API wrapper (`apps/chat/src/server-api/share.api.ts`'s `discardSharedCatalogItem(item.id)`, itself a thin delegate to the generated `@epam/chat-api-client` method).
...
- On success, closes the details panel via the existing `onCloseDetails`/`onClose` callback already used by the delete flow, since the item that was open has just been removed from the caller's catalog.
- Shows a success notification (`catalog.details.unshare.successTitle`/`catalog.details.unshare.success`) on success, and an error notification (`catalog.details.unshare.error`) on failure, mirroring `handleDelete`'s existing notification pattern.
- On failure: does NOT call refetch, does NOT clear selection, does NOT close the panel, and re-throws so the confirmation popup's pending state resolves correctly — the item remains visible in the catalog.
...
- **THEN** `refetchDeployments()` is called, the item no longer appears in `items` on the next render, a success notification is shown, and the details panel closes
...
- **THEN** `refetchToolsets()` is called (not `refetchDeployments`), the item no longer appears in `toolsets` on the next render, a success notification is shown, and the details panel closes
...
- **THEN** `setSelectedItemId(null)` is called before the details panel closes, and `DeploymentsContext`'s existing fallback chain determines the next selection (or `null` if the catalog is now empty) once `items` refreshes
...
- **THEN** no refetch is triggered, the item remains in the catalog list and in the currently open details panel, `setSelectedItemId` is not called, the panel is not closed, and an error notification is shown
