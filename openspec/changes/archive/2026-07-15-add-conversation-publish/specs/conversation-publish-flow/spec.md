...
New keys SHALL be added to a `ConversationPublishI18nKeys` enum in `apps/chat/src/constants/translation-keys.ts`, with English defaults in `apps/chat/src/i18n/locales/en.json` and matching entries added to every other locale file. Generic strings already covered by `ButtonsI18nKeys.Cancel`/`ButtonsI18nKeys.Publish` (if reused verbatim) SHALL be reused rather than re-declared, per the project's existing duplicate-value convention.

New keys (non-exhaustive — implementation SHALL add any additional strings needed, following this naming pattern):

| Key | English value |
|---|---|
| `conversationPanel.publishLabel` | "Publish" |
| `conversationPublish.title` | "Publish" |
| `conversationPublish.panelAriaLabel` | "Publish conversation" |
| `conversationPublish.submitLabel` | "Publish" |
...
- **THEN** `conversationPanel.publishLabel` resolves to "Publish"
