...
- [x] 1.1 In `apps/chat/src/components/ModelPicker/ModelPickerPanel.tsx`, extend the `talkableItems` `useMemo` filter to also allow `CatalogEntityType.Agent`, alongside the existing `Model` and `Agent` checks.
...
- [x] 2.1 Add/update a unit test in `apps/chat/src/components/ModelPicker/tests/` asserting a favorited item with `type: CatalogEntityType.Agent` is included in the rendered dropdown list.
