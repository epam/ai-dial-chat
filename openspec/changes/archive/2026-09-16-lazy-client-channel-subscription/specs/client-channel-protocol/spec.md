...
### Requirement: Eager mount-time and route-return subscription

**Reason**: Superseded by the demand-driven lifecycle. This behavior was never a standalone requirement heading — it was expressed inside the `liveChatInteraction` gate requirement as "the connect/reconnect/visibility-resume logic SHALL require both the flag being enabled AND the route condition", "returning to a streaming-capable route (flag still enabled) SHALL reconnect it", and its "Navigating back to a streaming-capable page reconnects" scenario. It is listed here so the removal is explicit rather than buried in a MODIFIED diff: opening or returning to a streaming-capable route, resolving the flag to enabled, and a tab becoming visible no longer open a subscription. The archived changes that introduced this behavior (`openspec/changes/archive/2026-09-07-client-channel-idle-disconnect/`, and the route-scoping work before it) remain accurate records of what the system did at that time and are not rewritten.

**Migration**: No caller migration is required — no API, DTO, generated-client, metric, or feature-flag surface changes, and both `useConversationStream` call sites already invoke `ensureConnected`/`waitForChannel` at the start of every completion, which is now what connects the channel. Anything that previously relied on a channel merely existing after navigation must instead request a completion; the channel-id-propagation requirement's bounded wait covers the resulting cold subscribe. To restore the previous connection profile without reverting the defect fixes, seed standing demand while the flag/route condition holds (see `design.md` §Migration Plan).
