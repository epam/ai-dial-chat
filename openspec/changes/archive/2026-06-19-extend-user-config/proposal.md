...
- **NEW** Render `<DialSpinner />` while user config is loading (matching the existing `RequireAuth` pattern), preventing dependent features from rendering with incorrect default values.
