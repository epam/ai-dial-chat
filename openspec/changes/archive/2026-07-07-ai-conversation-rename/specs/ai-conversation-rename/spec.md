...
The endpoint SHALL be rate limited per user via `@Throttle`, and SHALL translate upstream/LLM failures into typed HTTP exceptions rather than returning `null` or an empty name.

#### Scenario: Upstream LLM failure

- **WHEN** the DIAL Core / utility-model call fails or returns an unusable response
- **THEN** the system responds with a typed error status (e.g. 502/503) and logs the failure context without leaking secrets
