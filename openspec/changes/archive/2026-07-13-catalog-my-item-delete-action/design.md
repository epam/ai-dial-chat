...
New `components/Details/Header/DeleteButton/DeleteButton.tsx` owns only the trigger button and `isDeleting` state, and awaits the `onDelete(item): Promise<void> | void` prop passed down from `DetailsPanelProps`/`CatalogProps` (same shape as `onLogin`/`onLogout`). Clicking the button calls `onDelete` directly — there is no confirmation popup. While pending, the button is disabled; on settling (success or rejection) it re-enables. `onDeleted?.()` is called only when `onDelete` resolves. The component renders no error UI of its own — a rejected `onDelete` is the calling app's responsibility to surface (e.g. via a notification).

**Alternative considered**: a confirmation popup before calling `onDelete` (mirroring `CredentialsSection`'s logout confirmation and `DeleteAllConversationsAction`'s pattern), with an inline error message inside the lib. This was the original design, but the user explicitly requested removing both the confirmation step and the inline error state — Delete is now a single click with no popup and no in-lib error UI, same interaction weight as Edit/Share. Failure feedback moved entirely to the app layer (`CatalogView` shows a notification).
