...
- Create → `NotifiableEntity.Agent` + `EntityOperation.Created`, `name` = the application's name.
- Save → `NotifiableEntity.Agent` + `EntityOperation.Edited`, same `name`.
...
- **THEN** a success notification titled `"Agent created successfully"` naming the app is shown and the editor navigates to the return URL

#### Scenario: Save confirms and returns

- **WHEN** a user saves an existing custom app and the PATCH succeeds
- **THEN** a success notification titled `"Agent edited successfully"` naming the app is shown and the editor navigates to the return URL
