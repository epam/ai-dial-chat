...
- Add a **"Paused"** badge to the catalog card (`ScheduledTaskCard`, `libs/scheduled-tasks`), read-only and derived from the same `ScheduledTaskDto.isActive` field this change introduces. `ScheduledTaskItem` gains an `isActive?: boolean` field; the card renders the badge (with a pause icon) in place of the schedule pill when `isActive === false`, and the schedule pill as usual otherwise (`true` or `undefined`). `apps/chat/src/utils/map-scheduled-task-dto.ts`'s `mapScheduledTaskDtoToItem` maps `ScheduledTaskDto.isActive` straight through with no re-derivation — the BFF mapper (bullet above) remains the single source of the derivation logic. This is display-only: the catalog card issues no pause/resume request itself (that stays confined to the detail-page switch above).
...
- Interactive pause/resume switches on Scheduled Tasks catalog cards, bulk pause/resume, or automatic polling of active state. (A read-only "Paused" badge on the catalog card — display only, no mutation — is in scope; see the `scheduled-tasks-page-ui` capability below.)
...
- `scheduled-tasks-page-ui`: the catalog card (`ScheduledTaskCard`, `libs/scheduled-tasks`) renders a read-only "Paused" badge in place of the schedule pill when `item.isActive` is `false`, sourced from the same `ScheduledTaskDto.isActive` field this change adds — the catalog gets a display-only reflection of the same active state, with no toggle control of its own (that stays confined to the detail page, per the non-goal above).
...
- **Library**: `libs/scheduled-tasks/src/components/ScheduledTaskDetailView/` (new props: `isActive?`, `isActiveUpdating?`, `onActiveChange?`), `libs/scheduled-tasks/src/models/scheduled-task-detail-view-props.ts` (or equivalent) — no new host/API imports, preserving library isolation. Also `libs/scheduled-tasks/src/components/ScheduledTaskCard/` (new `isActive?` on `ScheduledTaskItem`, "Paused" badge rendering, matching `*Colors`/`*Typography`/`*Labels` props) and `apps/chat/src/utils/map-scheduled-task-dto.ts` (`mapScheduledTaskDtoToItem` maps `ScheduledTaskDto.isActive` straight through, no re-derivation).
