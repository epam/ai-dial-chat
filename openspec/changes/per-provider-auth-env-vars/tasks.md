...
- [x] 1.3 Remove the `AUTH_PROVIDERS!: string` field and its validation from `EnvironmentVariables`.
...
- [x] 2.2 Update `ProviderRegistryService.onModuleInit()` (`apps/chat-api/src/auth/providers/provider-registry.service.ts`) to call all 9 builders, filter out `undefined` results, keep the existing `validateSync(ProviderConfig)` pass per assembled entry, and remove the `JSON.parse(AUTH_PROVIDERS)` path entirely.
