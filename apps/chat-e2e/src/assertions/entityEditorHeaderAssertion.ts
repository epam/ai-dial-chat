...
import { BaseElement, EntityEditorHeader } from '@/src/ui/webElements';
import { ThemesUtil } from '@/src/utils/themesUtil';

export class EntityEditorHeaderAssertion extends BaseAssertion {
  readonly entityEditorHeader: EntityEditorHeader;

  constructor(entityEditorHeader: EntityEditorHeader) {
    super();
    this.entityEditorHeader = entityEditorHeader;
  }
  ...
  public async assertStepIsSelected(
    ...
    if (isSelected) {
      await this.assertElementState(
        this.entityEditorHeader.selectedIcon(stepLocator),
        'visible',
        `Step "${stepLocator}" should have selected icon`,
      );
      await this.assertElementState(
        this.entityEditorHeader.notSelectedIcon(stepLocator),
        'hidden',
  ...
  public async assertActiveStepIconState(
    ...
    const stepLocator = this.getStepLocator(step);
    const iconElement =
      this.entityEditorHeader.selectedFilledPointIcon(stepLocator);
  ...
  public async assertNotActiveStepIconState(
    ...
    const stepLocator = this.getStepLocator(step);
    const iconElement =
      this.entityEditorHeader.notSelectedPointIcon(stepLocator);
    await this.assertElementState(iconElement, expectedState);
    await this.assertElementColor(
      iconElement,
  ...
  public async assertCompletedStepIconState(
    ...
    const stepLocator = this.getStepLocator(step);
    const iconElement =
      this.entityEditorHeader.notSelectedCheckedCircleIcon(stepLocator);
    await this.assertElementState(iconElement, expectedState);
    await this.assertElementColor(
      iconElement,
      ThemesUtil.getRgbColorByKey(ThemeColorAttributes.textAccentPrimary),
  ...
  private getStepLocator(step: BaseElement | string) {
    return typeof step === 'string'
      ? this.entityEditorHeader.getStepByTitle(step)
      : step;
  }
}
