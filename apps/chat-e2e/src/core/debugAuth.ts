...
export class DebugAuth {
  ...
  private async submitUserCredentials(
    ...
    // The redirect_uri is generated by next-auth based on the callbackUrl, which we hardcoded.
    // However, next-auth uses the host of the original request (localhost) to build the state,
    // so we must replace it with the staging host to match what Auth0 expects.
