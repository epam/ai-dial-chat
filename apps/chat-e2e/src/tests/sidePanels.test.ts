...
test(
  ...
  async ({ dialHomePage, setTestIds, chatBar, header, promptBar }) => {
    ...
    await test.step('Hide chat panel', async () => {
    ...
    await test.step('Hide prompts panel', async () => {
    ...
    await test.step('Refresh page and verify both panels are hidden', async () => {
...
test(
  ...
  async ({
    ...
    await test.step('Open app, hover over resize chat panel icon and verify it is highlighted', async () => {
      await dialHomePage.openHomePage();
      await dialHomePage.waitForPageLoaded();
    ...
    await test.step('Resize chat panel to max size and verify max width', async () => {
    ...
    await test.step('Verify Attachment icon is visible at the panel bottom menu', async () => {
    ...
    await test.step('Hover over resize prompt panel icon and verify it is highlighted', async () => {
    ...
    await test.step('Resize prompt panel to max size and verify max width', async () => {
    ...
    await test.step('Hide both panels, open again and verify panels size is stored', async () => {
      for (let i = 1; i <= 2; i++) {
        await header.chatPanelToggle.click();
        await header.promptsPanelToggle.click();
      }
      const openedChatBarPanelBounding = await chatBar.getElementBoundingBox();
    ...
    await test.step('Refresh page and verify panels size is stored', async () => {
      await dialHomePage.reloadPage();
      const openedChatBarPanelBounding = await chatBar.getElementBoundingBox();
      const openedPromptsBarPanelBounding =
        await promptBar.getElementBoundingBox();
    ...
    await test.step('Resize chat panel to min size and verify min width', async () => {
      await chatBar.resizePanelWidth(appBounding!.x);
      const chatBarBounding = await chatBar.getElementBoundingBox();
      expect
        .soft(chatBarBounding!.width, ExpectedMessages.sideBarPanelWidthIsValid)
        .toBeCloseTo(ExpectedConstants.minSidePanelWidthPx, 0);
    });

    await test.step('Verify dots menu is visible at the panel bottom menu', async () => {
      ...
      const dotsMenuIconColor =
        await chatBar.bottomDotsMenuIcon.getComputedStyleProperty(Styles.color);
      expect
        .soft(dotsMenuIconColor[0], ExpectedMessages.iconColorIsValid)
        .toBe(Colors.controlsBackgroundDisable);
    });

    await test.step('Resize prompt panel to min size and verify min width', async () => {
