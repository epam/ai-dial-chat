...
| `MAX_GENERATION_DURATION_MS`            | `1800000`                      | Maximum time (milliseconds) an active generation may occupy the in-memory generation registry before the backend requests its cancellation, persists the partial response as an `Error` outcome, and releases the registry entry. Independent of the client connection — closing/refreshing/navigating away from the originating browser tab has no effect on the generation, so this timeout is what bounds a stalled upstream stream that never reaches `[DONE]`/error (default 30 min).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `GENERATION_FINALIZE_TIMEOUT_MS`        | `60000`                        | Bounds attach-subscriber and resource release, never registry-key ownership, for a generation whose terminal write was dispatched but never settled: past this many milliseconds the entry releases its subscribers, timer, and runtime tracking (reported as `state="settling"` on `dial.chat.generations.active`) but keeps its registry key, so a replacement is never admitted over an unsettled write. Recovery is the write settling, or a process restart.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
