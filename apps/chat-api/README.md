...
| `HALLOWEEN_ENABLED`                     | `false`                        | Client-visible feature flag for the Halloween easter egg in the chat app: cobwebs, drifting bats and a pumpkin on the empty-chat screen, where clicking the pumpkin sends a flock of ghosts drifting across the viewport, plus the secret phrase "trick or treat" typed into the conversation input, which drops a swarm of abseiling spiders instead of sending the message. Purely decorative — it gates no data, persists nothing, and changes no chat behavior; while `false` the phrase is an ordinary message and none of the decoration renders. Every animation it adds is suppressed under `prefers-reduced-motion: reduce`. Takes effect on the next service restart.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
