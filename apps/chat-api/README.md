...
| `AGENT_BUILDER_DEFAULT_MODEL` | `gpt-4o` | Agent Builder default model, exposed as `config.agentBuilderDefaultModelId` by client-config. Saved/manual selections take priority; if unavailable, the frontend selects the first tools-capable model. Independent of `DEFAULT_DEPLOYMENT`. Restart the BFF after changing; no frontend rebuild is required. |
