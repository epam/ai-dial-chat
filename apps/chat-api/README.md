...
| `CHAT_VERSION`                          | app `package.json` version     | Version string shown in the footer's corner version label and substituted for the `%%VERSION%%` footer token. Set from CI/CD to surface the deployed build. Blank or unset falls back to the app's `package.json` version, so a version label is always shown. Exposed to clients as `config.appVersion`; not role-gated.                                                                                                                                                                                                                                                            |
