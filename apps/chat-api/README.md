...
| `ALLOWED_IFRAME_ORIGINS`          | —                       | Comma-separated list of origins that may be loaded in iframes by this app (added to the CSP `frame-src` directive). Each entry must be a full URL with an `https://` (or `http://` for local development only) scheme. Needed when embedding external tools such as Quick Apps editors. Example: `https://quickapps.example.com`                                                                        |
