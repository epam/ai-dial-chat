...
describe('DeploymentsDetailsService', () => {
  ...
  describe('getDeploymentDetails', () => {
    ...
    it('maps themeUrl from catalog_properties alongside the other keys', async () => {
      const { service, sdkClient } = makeService();
      sdkClient.getApplication.mockResolvedValue(
        okResponse({
          id: 'applications/als-test-catalog',
    ...
    it('maps themeUrl when it is the only catalog property', async () => {
      const { service, sdkClient } = makeService();
      sdkClient.getApplication.mockResolvedValue(
        okResponse({
          id: 'applications/als-test-catalog',
      ...
      expect(result.applicationDetails?.catalogProperties?.themeUrl).toBe(
        'https://themes.contoso.example.com',
      );
    });

    it('ignores a non-string themeUrl', async () => {
      const { service, sdkClient } = makeService();
      sdkClient.getApplication.mockResolvedValue(
        okResponse({
          id: 'applications/als-test-catalog',
          catalog_properties: { themeUrl: 42 },
        }),
      );
