...
describe('ConversationService', () => {
  ...
  describe('listConversations', () => {
    ...
    it('normalizes the folder path for user and public bucket calls', async () => {
      const spy = vi
        .spyOn(service['dialClient'].client, 'getConversationMetadata')
        .mockImplementation(
          () => Promise.resolve({ data: { items: [] } }) as never,
    ...
    it('does not append a second slash to an already normalized folder path', async () => {
      const spy = vi
        .spyOn(service['dialClient'].client, 'getConversationMetadata')
        .mockImplementation(
          () => Promise.resolve({ data: { items: [] } }) as never,
    ...
    it('returns an empty list instead of 404 when the user bucket has no blobs under a scoped path', async () => {
      vi.spyOn(
        service['dialClient'].client,
        'getConversationMetadata',
      ).mockImplementation((bucket: string) => {
    ...
    it('still throws NotFoundException for a 404 on the user bucket root (empty path)', async () => {
      vi.spyOn(
        service['dialClient'].client,
        'getConversationMetadata',
      ).mockImplementation((bucket: string) => {
