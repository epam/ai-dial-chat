...
describe('ConversationService', () => {
  ...
  describe('streamPreviewCompletion', () => {
    it('streams chunks without reading or writing any persisted conversation', async () => {
      const getConversationSpy = vi.spyOn(service['client'], 'getConversation');
      const saveConversationSpy = vi.spyOn(
        service['client'],
        'saveConversation',
      );
      ...
      await service.streamPreviewCompletion(
        'applications/my-app',
    ...
    it('throws a mapped exception without flushing headers when DIAL Core rejects the request', async () => {
      vi.spyOn(
        service['client'],
        'sendChatCompletionRequest',
      ).mockResolvedValue({
      ...
      await expect(
        service.streamPreviewCompletion(
          'applications/my-app',
          [{ role: 'user', content: 'Hello' }],
          'test-token',
          new AbortController().signal,
    ...
    it('ends the stream without throwing when the client aborts mid-generation', async () => {
      vi.spyOn(
        service['client'],
        'sendChatCompletionRequest',
      ).mockRejectedValue(
