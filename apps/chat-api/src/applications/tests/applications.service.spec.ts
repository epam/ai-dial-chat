...
/*
 * `themeUrl` lives in `catalog_properties`, which — unlike
 * `application_properties` — chat only partly owns. These pin the merge
 * semantics that keep an operator's catalog keys alive across an apps-editor
 * save.
 */
describe('ApplicationsService — themeUrl on catalog_properties', () => {
  const THEME_URL = 'https://themes.contoso.example.com';
  const APP_PATH = 'applications/test-bucket/My%20App__0.0.1';

  const mockCreateSdk = (service: ApplicationsService) => {
    vi.spyOn(
      (service['dialClient'] as DialClientService).client,
      'getUserBucket',
    ).mockResolvedValue(okResponse({ bucket: 'test-bucket' }));
    return vi
      .spyOn(
        (service['dialClient'] as DialClientService).client,
        'saveCustomApplication',
  ...
  const mockUpdateSdk = (
    ...
    vi.spyOn(
      (service['dialClient'] as DialClientService).client,
      'getCustomApplication',
    ).mockResolvedValue(okResponse(existing));
    return vi
      .spyOn(
        (service['dialClient'] as DialClientService).client,
        'saveCustomApplication',
  ...
  const sentBodyOf = (spy: { mock: { calls: unknown[][] } }) =>
    (spy.mock.calls[0][2] as { body: Record<string, unknown> }).body;

  it('writes catalog_properties.themeUrl on create', async () => {
    const { service } = makeService();
    const save = mockCreateSdk(service);

    await service.createApplication('user1', 'token-abc', {
      name: 'My App',
      themeUrl: THEME_URL,
    });

    expect(sentBodyOf(save).catalogProperties).toEqual({
      themeUrl: THEME_URL,
    });
  });

  it('writes no catalog_properties on create when themeUrl is omitted', async () => {
    const { service } = makeService();
    const save = mockCreateSdk(service);
    ...
    expect(sentBodyOf(save).catalogProperties).toBeUndefined();
  });

  it('sets themeUrl on update while preserving other catalog keys', async () => {
    const { service } = makeService();
    const save = mockUpdateSdk(service, {
      displayName: 'My App',
      catalogProperties: { provider: 'Contoso' },
    });

    await service.updateApplication('user1', 'token-abc', APP_PATH, {
      name: 'My App',
      themeUrl: THEME_URL,
    });
  ...
  it('leaves catalog_properties untouched when themeUrl is omitted', async () => {
    const { service } = makeService();
    const stored = { provider: 'Contoso', themeUrl: THEME_URL };
    const save = mockUpdateSdk(service, {
      displayName: 'My App',
      catalogProperties: stored,
    });
    ...
    expect(sentBodyOf(save).catalogProperties).toEqual(stored);
  });

  it('deletes only themeUrl when an empty string is sent', async () => {
    const { service } = makeService();
    const save = mockUpdateSdk(service, {
      displayName: 'My App',
      catalogProperties: { provider: 'Contoso', themeUrl: THEME_URL },
    });
  ...
  it('writes an empty object when clearing the only catalog key', async () => {
    const { service } = makeService();
    const save = mockUpdateSdk(service, {
      displayName: 'My App',
      catalogProperties: { themeUrl: THEME_URL },
    });
  ...
  it('does not read or write application_properties when setting themeUrl', async () => {
    const { service } = makeService();
    const storedAppProperties = { tool_sets: ['toolset-1'] };
    const save = mockUpdateSdk(service, {
      displayName: 'My App',
      application_properties: storedAppProperties,
    });
    ...
    const body = sentBodyOf(save);
    expect(body.application_properties).toEqual(storedAppProperties);
    expect(body.catalogProperties).toEqual({ themeUrl: THEME_URL });
  });
});
