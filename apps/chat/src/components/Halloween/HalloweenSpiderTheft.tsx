...
const HalloweenSpiderTheft: FC = () => {
  ...
  useEffect(() => {
    ...
    const prepare = async () => {
      try {
        ...
        if (disposed) return;
        const targets = getSpiderTheftTargets(CONVERSATION_INPUT_CLASS);
        ...
        /* A missing lazy composer must still leave the decorative drop available. */
      }
    };
    prepare();
    return () => {
  ...
  return (
    <div
      ...
      data-halloween-scene={HalloweenBurst.Spiders}
      aria-hidden="true"
    >
      {!plans || reducedMotion
