...
const HalloweenRavens: FC = () => {
  ...
  useEffect(() => {
    ...
    const prepare = async () => {
      ...
      try {
        ...
        composerClass = CONVERSATION_INPUT_CLASS.wrapper;
      } catch {
        /* History and pumpkin anchors still support hosts without a composer. */
      }
      if (!disposed)
        setPrepared({
          plan: buildRavenPlan(getRavenTargets(composerClass), isMobile),
          isMobile,
          reducedMotion,
        });
    };
  ...
  return (
    <div
      ...
      aria-hidden="true"
      data-halloween-scene={HalloweenBurst.Ravens}
      data-ready={!!plan}
      data-animated={
        !reducedMotion &&
        plan?.active &&
