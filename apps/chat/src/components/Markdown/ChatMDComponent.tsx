...
export const getMDComponents = (
  ...
  return {
    ...
    th({ children }) {
      return (
        <th className="break-words border border-tertiary bg-layer-4 px-3 py-1 text-sm text-secondary-bg-dark">
          {children}
        </th>
      );
    },
...
const ChatMDComponent = ({
  ...
  const mdClassNames = classnames(
    'text-primary-bg-dark min-w-full prose-a:text-accent-primary prose-a:no-underline hover:prose-a:underline',
    {
