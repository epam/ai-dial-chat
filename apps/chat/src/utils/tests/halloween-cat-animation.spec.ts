  ...
  plan = buildCatPlan(
    {
      ...
      buttons: [
        {
          element: fixture.querySelector<HTMLElement>('#first')!,
          rect: new DOMRect(440, 470, 40, 40),
        },
        {
          element: fixture.querySelector<HTMLElement>('#second')!,
          rect: new DOMRect(492, 470, 40, 40),
        },
      ],
      supports: [
        {
          element: fixture.querySelector<HTMLElement>('#support')!,
          rect: new DOMRect(420, 570, 300, 44),
        },
      ],
    },
    false,
...
describe('cat gravity animation ownership', () => {
  ...
  it('synchronizes cat and copies while limiting original animations to temporary opacity', () => {
    ...
    for (const { target, entry } of plan.buttons) {
      ...
      original.frames.forEach((frame) =>
        expect(frame.transform).toBeUndefined(),
      );
    }
    expect(
      recorded.find(({ element }) => element.id === 'first')?.frames.at(-1)
        ?.opacity,
    ).toBe('0.7');
  });
  ...
  it('preserves contact-specific keyframe easing', () => {
    ...
    play();
    expect(
      recorded.find(({ element }) => element === actor)?.frames[0].easing,
    ).toBe('linear');
    expect(
      recorded.find(
        ({ element }) =>
          element.getAttribute('data-cat-part') === CatPart.Forearm,
      )?.frames[0].easing,
    ).toBe('steps(1, end)');
  });
  ...
  it.each([
    ...
    else if (reason === 'button expanded')
    ...
    expectStopped();
    if (reason === 'button text')
      expect(target.textContent).toBe('Changed label');
    if (reason === 'hidden ancestor') expect(fixture.hidden).toBe(true);
  });

  it.each(['already visible', 'appears during playback'])(
    'survives a fixed notification that is %s and disappears after five seconds',
    async (appearance) => {
  ...
  it.each([false, true])(
    'handles an ancestor sibling insertion with actual layout shift=%s',
    async (shifted) => {
      play();
  ...
  it.each(['actor', 'expression', 'snapshot', 'original opacity'])(
    'rolls back partial setup when %s animation fails',
    (failure) => {
      const before = fixture.outerHTML;
      const animate = vi
