...
describe('raven UI targets', () => {
  it('finds one visible conversation, composer and pumpkin without touching focus or contents', () => {
    ...
    const targets = getRavenTargets('composer');
  ...
  it.each([
    ...
    expect(getRavenTargets('composer').conversation).toBeNull();
  });
  ...
  it('ignores hidden ancestors and can read physical RTL placement', () => {
    ...
    expect(getRavenTargets('composer').conversation?.rect.left).toBe(1010);
    fixture.style.visibility = 'hidden';
    expect(getRavenTargets('composer')).toMatchObject({
      conversation: null,
      pumpkin: null,
      composer: null,
      fragments: [],
  ...
  it('never copies large controls, editors or the focused fragment', () => {
    ...
    focused.focus();
    expect(getRavenTargets('composer').fragments).toHaveLength(0);
  });
  ...
  it('bounds history discovery and caches rectangle reads during setup', () => {
    ...
    rows[0].hidden = false;
    expect(getRavenTargets('composer').conversation?.element).toBe(rows[0]);
