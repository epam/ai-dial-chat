import { getCelebrationHistoryRows } from './celebration-history';
import {
...
export const getSpiderTheftTargets = (
  ...
  const wrapper = Array.from(
    document.querySelectorAll<HTMLElement>(`.${classes.wrapper}`),
  ).find((element) => !element.closest('[inert], [aria-hidden="true"]'));
  const welcome = wrapper?.closest('[role="region"]');
  const candidates = [
    welcome?.querySelector<HTMLElement>('h1') ?? null,
    wrapper?.querySelector<HTMLElement>(`.${classes.modelSelectorButton}`) ??
      null,
    wrapper?.querySelector<HTMLElement>(`.${classes.addCluster}`) ?? null,
  ]
    .map(visibleTarget)
    .filter((target): target is CelebrationSnapshotTarget => target !== null);
  const history = shuffled(getCelebrationHistoryRows());
  const selection = [
