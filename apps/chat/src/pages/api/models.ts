...
const webRag = {
  ...
  isDefault: false,
  description:
    'This application allows you to search and retrieve information from the web in real-time. It combines the power of retrieval and generative models to provide accurate and contextually relevant answers.',
  type: 'application',
  features: {
    systemPrompt: true,
    truncatePrompt: false,
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    return res.status(200).json([...entities, webRag, hrBuddy]);
  } catch (error) {
    logger.error(error);
