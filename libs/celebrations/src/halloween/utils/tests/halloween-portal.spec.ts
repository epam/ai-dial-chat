...
const borrow = () => {
  const stop = animatePortalRows(getCelebrationHistoryRows(anchors), host, {
    x: 500,
    y: 300,
  });
  stops.push(stop);
...
describe('portal history illusion', () => {
  ...
  it('uses real visible history rows and skips focused, clipped and closed rows', () => {
    ...
    expect(
      getCelebrationHistoryRows(anchors).map(({ element }) => element),
    ).toEqual([visible.row]);
    panel.style.overflow = 'hidden';
    vi.spyOn(panel, 'getBoundingClientRect').mockReturnValue(
      bounds(0, 0, 324, 170),
    );
    expect(getCelebrationHistoryRows(anchors)).toHaveLength(0);
  });
