...
const CELEBRATION_HISTORY_CLASS = 'celebration-history';
...
const scenes = [HalloweenScene.Cauldron, HalloweenScene.Mimic];
const play = (scene = HalloweenScene.Cauldron) => {
  const stop = animateSecretSceneHistory(
    scene,
    getSecretSceneTargets(anchors),
    host,
    stage,
  );
  stops.push(stop);
...
describe('secret scene history interactions', () => {
  ...
  it('borrows visible rows for the cauldron scene', () => {
    expect(
      getSecretSceneTargets(anchors).map(({ element }) => element.tagName),
    ).toEqual(['LI', 'LI']);
  });
  ...
  it.each(['focus', 'expanded', 'hidden', 'closed', 'clipped', 'empty'])(
    ...
    (reason) => {
      ...
      expect(getSecretSceneTargets(anchors)).toHaveLength(
  ...
  it('does not mistake expanded history groups for an open action menu', () => {
    ...
    panel.prepend(group);
    expect(getSecretSceneTargets(anchors)).toHaveLength(2);
  });
