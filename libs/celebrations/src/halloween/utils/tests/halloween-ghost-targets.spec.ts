...
describe('ghost possession targets', () => {
  ...
  it('does not clone drafts or disturb focus and allows the focused pumpkin reaction anchor', () => {
    ...
    const targets = getGhostTargets(testAnchors({ composer: 'composer' }));
    ...
    pumpkin.focus();
    const focused = getGhostTargets(testAnchors({ composer: 'composer' }));
  ...
  it.each([
    ...
    expect(
      getGhostTargets(testAnchors({ composer: 'composer' })).homes.map(
        ({ element }) => element,
      ),
  ...
  it('never selects overlapping, neighboring or ancestor-related homes together', () => {
    ...
    const homes = getGhostTargets(testAnchors({ composer: 'composer' })).homes;
  ...
  it('uses fewer homes when the layout is sparse and has no phantom pumpkin', () => {
    ...
    expect(getGhostTargets(testAnchors({ composer: '' }), 3)).toEqual({
      width: 1280,
      height: 900,
      pumpkin: undefined,
      homes: [],
  ...
  it('bounds history discovery and measures each visited element only once', () => {
    ...
    rows[0].hidden = false;
    getGhostTargets(testAnchors({ composer: 'composer' }));
