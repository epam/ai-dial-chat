...
export const getHalloweenWebTargets = (
  ...
  /* An empty class name matches nothing, so missing anchors add no targets. */
  ...
  for (
    ...
    add(
      wrapper.getElementsByClassName(
        anchors.composerAddCluster ?? '',
      )[0] as HTMLElement,
    );
    if (result.length) {
      add(
        findWelcomeRegion(wrapper, anchors)?.querySelector<HTMLElement>('h1'),
      );
      break;
    }
  }
