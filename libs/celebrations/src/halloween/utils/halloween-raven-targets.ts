...
export const getRavenTargets = (anchors: CelebrationAnchors): RavenTargets => {
  const { composer: composerClass = '' } = anchors;
  ...
  const history =
    queryAllOrNone(getHistoryContainerSelector(anchors))[0] ?? null;
  const candidates = [
