...
const Harness = ({
  onSubmit = vi.fn(),
  ...props
}: Partial<Omit<ScheduledTaskCreateFormProps, 'labels' | 'onSubmit'>> & {
  labels?: Partial<ScheduledTaskCreateFormProps['labels']>;
  onSubmit?: (values: ScheduledTaskCreateFormValues) => void;
}) => {
  const [values, setValues] = useState<ScheduledTaskCreateFormValues>({
    displayName: 'Task',
...
describe('draft replacement', () => {
  it('cancels an equal-text draft replacement and clears Undo', async () => {
    ...
    const { rerender } = render(
      <Harness onRefineDescription={refine} key="first" />,
    );
    await userEvent.click(action());
    rerender(<Harness onRefineDescription={refine} key="second" />);
    expect(refine.mock.calls[0][1].aborted).toBe(true);
    await act(async () => pending.resolve('Late'));
    expect(field().value).toBe('Original');
    expect(screen.queryByRole('button', { name: 'Undo' })).toBeNull();
