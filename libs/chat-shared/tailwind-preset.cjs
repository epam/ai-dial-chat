/*
 * AI DIAL design-token Tailwind preset.
 *
 * This is the single source of the semantic colour scale that every app and lib
 * in this workspace — and every consuming host — builds against. Library JSX
 * uses class names from it (`bg-layer-raised`, `text-secondary`,
 * `stroke-secondary`, …), so a host that does not apply this preset emits none
 * of those utilities and the libraries render unstyled.
 *
 * Published as `@epam/ai-dial-chat-shared/tailwind-preset`:
 *
 *   presets: [require('@epam/ai-dial-chat-shared/tailwind-preset')]
 *
 * It deliberately carries no `content` globs — those are specific to each
 * consumer. See the host setup section of openspec/lib-styling-guide.md.
 *
 * The `.cjs` extension is required, not stylistic: this package is ESM
 * (`"type": "module"`), and Tailwind configs are CommonJS loaded by tooling
 * outside the bundler. It carries no imports, so `chat-shared` stays a
