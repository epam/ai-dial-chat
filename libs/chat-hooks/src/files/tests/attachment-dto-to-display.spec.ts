...
/*
 * Stand-in resolvers with the same shape as the app's real
 * `attachmentDisplayResolvers` (catalog icon preview + DIAL download-URL
 * playback), so the spec doesn't depend on API endpoint constants.
 */
const resolvers: AttachmentDisplayResolvers = {
  resolvePreviewUrl: (dto) =>
    dto.url?.startsWith('files/')
      ? `/api/v1/files/download?path=${dto.url.slice('files/'.length)}`
      : dto.url,
  resolvePlayUrl: (dto) =>
    dto.url?.startsWith('files/')
      ? `/api/v1/files/download?path=${dto.url.slice('files/'.length)}`
...
describe('attachmentDtoToDisplayAttachment', () => {
  it('maps a file DTO to a display attachment without a local File', () => {
    ...
    const attachment = attachmentDtoToDisplayAttachment(dto, resolvers);
