...
describe('mimeTypesToFileAccept', () => {
  ...
  it('filters out a value that cannot be passed to DialFileManager instead of joining it unfiltered', () => {
    expect(mimeTypesToFileAccept(['pdf', 'application/pdf'])).toBe(
      'application/pdf',
    );
  });

  it('never disagrees with mimeTypesToDialFileAcceptTypes about which values are acceptable', () => {
    const input = ['pdf', 'application/pdf', '.csv', 'invalid'];
    const accept = mimeTypesToFileAccept(input);
    const dialFileAcceptTypes = mimeTypesToDialFileAcceptTypes(input) ?? [];

    expect(accept?.split(',').sort()).toEqual([...dialFileAcceptTypes].sort());
  });
});
