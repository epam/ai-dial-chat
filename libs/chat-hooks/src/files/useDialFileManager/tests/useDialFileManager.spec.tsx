...
import type { FileNameValidationError } from '../../dial-file-manager.types';
...
/** Hand-rolled `DialFilesApi` test double — replaces the old `server-api/files.api` module mock. */
...
let mockListSharedFiles: ReturnType<typeof vi.fn>;
let mockListPublicFiles: ReturnType<typeof vi.fn>;
let mockListSharedByMe: ReturnType<typeof vi.fn>;
let mockDownloadArchive: ReturnType<typeof vi.fn>;
let mockDownloadFile: ReturnType<typeof vi.fn>;
let mockDeleteFiles: ReturnType<typeof vi.fn>;
let mockCopyFiles: ReturnType<typeof vi.fn>;
let mockMoveFiles: ReturnType<typeof vi.fn>;
let mockDiscardShared: ReturnType<typeof vi.fn>;
let mockRevokeAccess: ReturnType<typeof vi.fn>;
let mockGetFileMetadata: ReturnType<typeof vi.fn>;

/** Replaces the old `useOperationNotification` spy — asserts the structured `onOperationSuccess` event instead. */
let mockOnOperationSuccess: ReturnType<typeof vi.fn>;
let mockOnNotification: ReturnType<typeof vi.fn>;

const defaultLabels: Partial<Record<DialFileManagerActions, string>> = {
...
const defaultDownloadDestination: DownloadDestinationHandlers = {
...
const defaultBuildValidationErrorMessage = (
  error: FileNameValidationError,
): string => error.reason;

/** Merges the four host-injected fixtures (filesApi/labels/locale/etc.) into every test's options. */
const buildOptions = (
  overrides: Partial<UseDialFileManagerOptions> &
    Pick<UseDialFileManagerOptions, 'bucket'>,
  ...
  buildValidationErrorMessage: defaultBuildValidationErrorMessage,
  onNotification:
    mockOnNotification as UseDialFileManagerOptions['onNotification'],
  onOperationSuccess:
    mockOnOperationSuccess as UseDialFileManagerOptions['onOperationSuccess'],
  ...overrides,
});

beforeEach(() => {
  filesApi = makeFilesApi();
  mockListFiles = vi.mocked(filesApi.listFiles);
  mockListSharedFiles = vi.mocked(filesApi.listSharedFiles);
  mockListPublicFiles = vi.mocked(filesApi.listPublicFiles);
  mockListSharedByMe = vi.mocked(filesApi.listSharedByMe);
  mockDownloadArchive = vi.mocked(filesApi.downloadArchive);
  mockDownloadFile = vi.mocked(filesApi.downloadFile);
  mockDeleteFiles = vi.mocked(filesApi.deleteFiles);
  mockCopyFiles = vi.mocked(filesApi.copyFiles);
  mockMoveFiles = vi.mocked(filesApi.moveFiles);
  mockDiscardShared = vi.mocked(filesApi.discardShared);
  mockRevokeAccess = vi.mocked(filesApi.revokeAccess);
  mockGetFileMetadata = vi.mocked(filesApi.getFileMetadata);
  mockOnOperationSuccess = vi.fn();
...
describe('useDialFileManager', () => {
  ...
  it('shows a notification when folder creation fails', async () => {
    ...
    const { result } = renderHook(() =>
      useDialFileManager(buildOptions({ bucket: BUCKET, onNotification })),
    );
    await waitFor(() => expect(result.current.isLoading).toBe(false));
  ...
  it('downloads multiple files using API paths derived from virtual paths', async () => {
    ...
    expect(mockOnOperationSuccess).toHaveBeenCalledWith({
  ...
  it('shows a notification when file download fails', async () => {
    ...
    await waitFor(() =>
      expect(onNotification).toHaveBeenCalledWith({
        variant: NotificationVariant.Error,
        reason: FileManagerNotificationReason.DownloadFilesFailed,
      }),
    );
  });
