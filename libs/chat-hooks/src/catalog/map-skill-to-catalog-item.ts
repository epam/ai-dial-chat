...
const SOURCE_FOLDER_LABEL: Record<SkillSource, keyof DeploymentFolderLabels> = {
  [SkillSource.Personal]: 'personal',
  [SkillSource.SharedWithMe]: 'shared',
...
export const mapSkillToCatalogItem = (
  ...
  { folderLabels, source, favoriteIds }: MapSkillToCatalogItemOptions,
): CatalogItem => {
...
const buildSpecificationSection = (
  ...
  if (about.whenToUse != null) {
    specs.push({ label: labels.whenToUseLabel, value: about.whenToUse });
  }
  if (about.allowedTools?.length) {
    specs.push({
      label: labels.allowedToolsLabel,
      value: joinSpecList(about.allowedTools),
    });
  }
  if (about.bundledResources?.length) {
    specs.push({
      label: labels.bundledResourcesLabel,
  ...
  return { title: labels.specificationSectionTitle, specs };
};
...
export const buildSkillOverview = (
  ...
  const specification = buildSpecificationSection(about, labels);
  ...
  if (skill?.author) {
    specs.push({ label: labels.authorLabel, value: skill.author });
  }
  specs.push({
  ...
  specs.push({
    label: labels.fileCountLabel,
    value: String(selectFileItems(files).length),
  });

  sections.push({ title: labels.detailsSectionTitle, specs });
