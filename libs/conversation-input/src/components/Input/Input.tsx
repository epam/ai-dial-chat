...
export const Input: FC<InputProps> = ({
   ...
   * keys an autocomplete owns before the textarea's own handling: the arrows
   * move the active option instead of navigating message history (the value
   * is a single-line `/query`, so there is no line to move to), and Enter
   * takes the active option instead of sending the `/query` to the model —
   * with no option active it does nothing rather than send. Shift+Enter and,
   * in MetaEnter mode, a bare Enter with nothing active keep inserting a
  ...
  const commandMenuArea =
    commandMenu == null ? (
      ...
      <Dropdown
        ...
        renderOverlay={() => {
          /*
           * The wrapper is always rendered: its ref scopes the keyboard's
           * option lookup to this menu. It becomes a labeled group only when
           * the host names the menu region.
           */
